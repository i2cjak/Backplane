...
}).annotate({ description: "Links an authenticated cloud user to a Backplane environment." });
...
>()("@backplane/contracts/relay/RelayClientPrincipal") {}
...
>()("@backplane/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Backplane Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["backplane-mobile", "backplane-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "backplane-mobile" as const;
export const RelayWebClientId = "backplane-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Backplane Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Backplane environments, connecting authorized clients, and publishing agent activity.",
  );
