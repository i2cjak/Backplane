...
>()("@backplane/contracts/environmentHttp/EnvironmentAuthenticatedPrincipal") {}
...
class EnvironmentMetadataHttpApi extends HttpApiGroup.make("metadata").add(
  HttpApiEndpoint.get("descriptor", "/.well-known/backplane/environment", {
...
class EnvironmentConnectHttpApi extends HttpApiGroup.make("connect")
  ...
  .add(
    HttpApiEndpoint.post("health", "/api/backplane-connect/health", {
  ...
  .add(
    HttpApiEndpoint.post("backplaneMintCredential", "/api/backplane-connect/mint-credential", {
      payload: RelayCloudMintCredentialRequest,
      success: RelayEnvironmentMintResponse,
      error: EnvironmentHttpCloudErrors,
    }),
