...
export const T3ProjectFileScript = Schema.Struct({
  name: trimmedNonEmpty({
    description: "Display name for the script, shown in the T3CAD scripts menu.",
  }),
  command: trimmedNonEmpty({
    description: "Shell command executed in a T3CAD terminal at the project root.",
  }),
  ...
  description: "A project script that team members can import into T3CAD.",
});
...
export const T3ProjectFile = Schema.Struct({
  ...
  iconPath: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          'Workspace-relative path to the project icon (e.g. "assets/logo.svg"). Checked before T3CAD\'s built-in icon locations.',
      },
  ...
  defaultThreadEnvMode: Schema.optionalKey(
    ThreadEnvMode.annotate({
      description:
        'Where new threads start for this repository: "worktree" for a fresh git worktree, "local" for the current checkout. A per-project setting in T3CAD overrides this; when neither is set, the global default applies.',
    }),
  ),
  scripts: Schema.optionalKey(
    Schema.Array(T3ProjectFileScript)
      .annotate({
        description: "Project scripts shared with everyone who opens this repository in T3CAD.",
      })
  ...
  description:
    "Checked-in project configuration for T3CAD (t3.json at the repository root). See https://t3.codes for documentation.",
});
