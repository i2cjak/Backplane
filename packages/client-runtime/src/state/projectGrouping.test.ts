import { EnvironmentId, ProjectId } from "@backplane/contracts";
...
const repositoryIdentity = {
  canonicalKey: "github.com/backplane/backplane",
  locator: {
    source: "git-remote" as const,
    remoteName: "upstream",
    remoteUrl: "https://github.com/backplane/backplane.git",
  },
  provider: "github",
  owner: "backplane",
  name: "backplane",
...
describe("buildProjectGroups", () => {
  it("preserves every physical clone as a selectable member in repository modes", () => {
    const projects = [
      makeProject("backplane", "/work/backplane"),
      makeProject("backplane-2", "/work/backplane-2"),
      makeProject("backplane-3", "/work/backplane-3"),
    ];
  ...
  it("uses a shared custom title as the repository group's label", () => {
    const projects = [
      makeProject("first", "/work/backplane", { title: "Custom project" }),
      makeProject("second", "/work/backplane-2", { title: "Custom project" }),
    ];
  ...
  it("keeps the repository label when shared titles match its repository name", () => {
    const projects = [
      makeProject("first", "/work/backplane", { title: "backplane" }),
      makeProject("second", "/work/backplane-2", { title: "backplane" }),
    ];
  ...
  it("keeps physical clones in separate groups when requested", () => {
    ...
    expect(groups.map((group) => group.label)).toEqual(["backplane", "backplane-2", "backplane-3"]);
  });

  it("applies a physical-project override without dropping its siblings", () => {
    const first = makeProject("backplane", "/work/backplane");
    const second = makeProject("backplane-2", "/work/backplane-2");
    const third = makeProject("backplane-3", "/work/backplane-3");
  ...
  it("dedupes stale registrations at one physical path using the freshest project", () => {
    const stale = makeProject("stale", "/work/backplane", {
      repositoryIdentity: null,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const fresh = makeProject("fresh", "/work/backplane/", {
  ...
  it("uses repository identity from a duplicate registration when the winner lacks it", () => {
    const identified = makeProject("identified", "/work/backplane", {
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshUnidentified = makeProject("fresh", "/work/backplane/", {
      repositoryIdentity: null,
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const sibling = makeProject("sibling", "/work/backplane-2");
  ...
  it("uses the freshest winner's repository identity when stale duplicates disagree", () => {
    const staleIdentity = {
      ...repositoryIdentity,
      canonicalKey: "github.com/backplane/old-repository",
      name: "old-repository",
  ...
  it("uses the freshest identity-bearing duplicate when the winner lacks identity", () => {
    ...
    const staleIdentified = makeProject("stale-identified", "/work/backplane", {
      repositoryIdentity: staleIdentity,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshIdentified = makeProject("fresh-identified", "/work/backplane/", {
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const winner = makeProject("winner", "/work/backplane", {
      repositoryIdentity: null,
