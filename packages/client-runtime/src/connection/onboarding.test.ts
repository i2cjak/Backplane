import { AuthStandardClientScopes, EnvironmentId } from "@backplane/contracts";
...
function pairingHttpLayer(
  ...
  const fetchFn = ((input, init = {}) => {
    ...
    if (url.endsWith("/.well-known/backplane/environment")) {
  ...
  it.effect("prepares a persisted bearer registration from pairing details", () =>
    Effect.gen(function* () {
      ...
      expect(calls.map((call) => call.url)).toEqual([
        "https://remote.example.test/.well-known/backplane/environment",
      ...
      expect(tokenParams.get("client_label")).toBe("Backplane Test");
    }),
  );
