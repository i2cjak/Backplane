...
} from "@backplane/shared/httpReadiness";
import * as NetService from "@backplane/shared/Net";
import { extractJsonObject, fromLenientJson } from "@backplane/shared/schemaJson";
import { satisfiesSemverRange } from "@backplane/shared/semver";
...
export interface RemoteBackplaneRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteBackplaneRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteBackplaneRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  BACKPLANE_NODE_ENGINE_RANGE=@@BACKPLANE_NODE_ENGINE_RANGE@@
  if [ -z "$BACKPLANE_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$BACKPLANE_NODE_ENGINE_RANGE" <<'NODE'
@@BACKPLANE_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for BACKPLANE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$BACKPLANE_NODE_BIN/node" ]; then
        PATH="$BACKPLANE_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@BACKPLANE_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
BACKPLANE_NODE_SCRIPT_PATH=@@BACKPLANE_NODE_SCRIPT_PATH@@
if [ -n "$BACKPLANE_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$BACKPLANE_NODE_SCRIPT_PATH" "$@"
fi
if command -v backplane >/dev/null 2>&1; then
  exec backplane "$@"
fi
# npm extracts a package before it runs the native builds of its dependencies,
# so a failed build (backplane depends on node-pty, which needs a C toolchain) leaves
# the npx cache without a backplane executable. \`npx --yes\` then exits 0 without
...
require_installed_backplane_cli() {
  if ! BACKPLANE_CLI_PATH="$("$@" -- sh -c 'command -v backplane')"; then
    printf 'Remote host could not install %s. See npm output above for the cause.\\n' @@BACKPLANE_PACKAGE_SPEC@@ >&2
    return 1
  fi
  if [ -n "$BACKPLANE_CLI_PATH" ]; then
    return 0
  fi
  printf 'Remote host installed %s but npm produced no backplane executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@BACKPLANE_PACKAGE_SPEC@@ >&2
  return 1
}
# The launcher records this PID, so exec the CLI without an npm wrapper process.
if command -v npx >/dev/null 2>&1; then
  require_installed_backplane_cli npx --yes --package @@BACKPLANE_PACKAGE_SPEC@@ || exit 1
  exec "$BACKPLANE_CLI_PATH" "$@"
fi
if command -v npm >/dev/null 2>&1; then
  require_installed_backplane_cli npm exec --yes --package @@BACKPLANE_PACKAGE_SPEC@@ || exit 1
  exec "$BACKPLANE_CLI_PATH" "$@"
fi
printf 'Remote host is missing the backplane CLI and could not install @@BACKPLANE_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@BACKPLANE_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.backplane/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.backplane"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-backplane.sh"
RUNNER_NEXT="$STATE_DIR/run-backplane.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@BACKPLANE_DEFAULT_REMOTE_PORT@@" "@@BACKPLANE_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@BACKPLANE_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@BACKPLANE_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@BACKPLANE_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@BACKPLANE_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@BACKPLANE_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@BACKPLANE_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env BACKPLANE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@BACKPLANE_READY_TIMEOUT_MS@@"; then
    printf 'Remote Backplane server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
STATE_DIR="$HOME/.backplane/ssh-launch/@@BACKPLANE_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.backplane"
RUNNER_FILE="$STATE_DIR/run-backplane.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
if [ "$REMOTE_MANAGED" != "external" ] && [ -n "$REMOTE_PID" ] && kill -0 "$REMOTE_PID" 2>/dev/null; then
  ...
  if kill -0 "$REMOTE_PID" 2>/dev/null; then
    printf 'Remote Backplane server with PID %s did not stop within 2 seconds. Its ownership files were kept.\\n' "$REMOTE_PID" >&2
    exit 1
  fi
fi
rm -f "$PID_FILE" "$PORT_FILE" "$MANAGED_FILE"
...
export function buildRemoteBackplaneRunnerScript(input?: RemoteBackplaneRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "@backplane/cli@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      BACKPLANE_PACKAGE_SPEC: packageSpec,
      BACKPLANE_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      BACKPLANE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemoteBackplaneRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      BACKPLANE_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      BACKPLANE_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteBackplaneRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    BACKPLANE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    BACKPLANE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteBackplaneRunnerScript(input)),
    BACKPLANE_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    BACKPLANE_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    BACKPLANE_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    BACKPLANE_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    BACKPLANE_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    BACKPLANE_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    BACKPLANE_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteBackplaneRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    BACKPLANE_STATE_KEY: remoteStateKey(target),
    BACKPLANE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteBackplaneRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteBackplaneRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteBackplaneRunnerOptions;
...
>()("@backplane/ssh/tunnel/SshEnvironmentManager") {
