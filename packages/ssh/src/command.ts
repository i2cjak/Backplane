...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@backplane/contracts";
...
const PUBLISHABLE_BACKPLANE_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteBackplaneCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_BACKPLANE_VERSION_PATTERN.test(appVersion)) {
    return `@backplane/cli@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "@backplane/cli@nightly";
  }

  return input.updateChannel === "nightly" ? "@backplane/cli@nightly" : "@backplane/cli@latest";
}
