...
import {
  ...
  resolveRemoteBackplaneCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote backplane package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemoteBackplaneCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "@backplane/cli@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemoteBackplaneCliPackageSpec({
