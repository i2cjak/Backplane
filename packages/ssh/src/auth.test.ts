  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({
        prefix: "backplane-ssh-askpass-test-",
      });
      ...
      assert.equal(env.BACKPLANE_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "backplane");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\backplane-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\backplane-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
