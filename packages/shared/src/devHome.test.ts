...
import { resolveGitWorktreePath, resolveWorktreeBackplaneHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "backplane-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeBackplaneHome", () => {
  it.effect("answers with .backplane before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeBackplaneHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".backplane"));
