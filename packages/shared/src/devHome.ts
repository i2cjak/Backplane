 ...
 * `~/.backplane` that a user's installed Backplane runs against.
 *
 * A linked git worktree gets its own (gitignored) `.backplane`: feature work in a
 * throwaway branch must not share a database with the real app, and an ambient
 * `BACKPLANE_HOME` counts as an explicit base dir — flipping the state directory
 ...
 */
export const resolveWorktreeBackplaneHome = (
  cwd: string,
): Effect.Effect<string | undefined, never, FileSystem.FileSystem | Path.Path> =>
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, ".backplane");
  });
