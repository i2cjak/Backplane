...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    >(() => "__BACKPLANE_ENV_PATH_START__\n/a:/b\n__BACKPLANE_ENV_PATH_END__\n");
    ...
    expect(args?.[1]).toContain("__BACKPLANE_ENV_PATH_START__");
    expect(args?.[1]).toContain("__BACKPLANE_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__BACKPLANE_ENV_PATH_END__",
        "__BACKPLANE_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__BACKPLANE_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      [
        "__BACKPLANE_ENV_CUSTOM_VAR_START__",
        "  padded value  ",
        "__BACKPLANE_ENV_CUSTOM_VAR_END__",
      ].join("\n"),
    );
...
describe("readEnvironmentFromWindowsShell", () => {
  ...
  it("strips CRLF delimiters from captured PowerShell values", () => {
    ...
    >(
      () =>
        "__BACKPLANE_ENV_FNM_DIR_START__\r\nC:\\Users\\testuser\\AppData\\Roaming\\fnm\r\n__BACKPLANE_ENV_FNM_DIR_END__\r\n",
    );
  ...
  it("falls back to Windows PowerShell when pwsh.exe is unavailable", () => {
    ...
    >((file) => {
      ...
      return "__BACKPLANE_ENV_PATH_START__\nC:\\Tools\n__BACKPLANE_ENV_PATH_END__\n";
    });
...
effectIt.layer(NodeServices.layer)("resolveCommandPath", (it) => {
  ...
  it.effect.each(["audit-command", "audit-command.CMD"])(
    ...
    (command) =>
      Effect.gen(function* () {
        ...
        const cwd = yield* fs.makeTempDirectoryScoped({ prefix: "backplane-case-sensitive-path-" });
  ...
  it.effect("keeps cached misses until expiry while allowing explicit paths", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const cwd = yield* fs.makeTempDirectoryScoped({ prefix: "backplane-path-cache-" });
