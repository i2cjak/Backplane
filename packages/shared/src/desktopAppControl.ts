 ...
 * The state directory is hashed so custom Backplane homes cannot exceed Unix socket
 * path limits.
 */
export function resolveDesktopAppControlAddress(input: {
  ...
  if (input.platform === "win32") {
    return {
      address: `\\\\.\\pipe\\backplane-app-${stateHash}`,
      directory: null,
    };
  }
  ...
  const directory = input.joinPath(input.tempDir, `backplane-${userKey}`);
  return {
