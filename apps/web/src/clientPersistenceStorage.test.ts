import { DEFAULT_CLIENT_SETTINGS } from "@backplane/contracts";
...
describe("clientPersistenceStorage", () => {
  ...
  it.each(["not-json", '{"wordWrap":"invalid"}'])(
    ...
    async (value) => {
      ...
      testWindow.localStorage.setItem("backplane:client-settings:v1", value);
      ...
      expect(() => readBrowserClientSettings()).toThrow(
        expect.objectContaining({
          ...
          operation: "decode",
          storageKey: "backplane:client-settings:v1",
        }),
      );
      expect(testWindow.localStorage.getItem("backplane:client-settings:v1")).toBe(value);
    },
  );
  ...
  it("preserves saved settings across a transient read failure", async () => {
    ...
    testWindow.localStorage.setItem("backplane:client-settings:v1", JSON.stringify(settings));
  ...
  it("keeps the diff layout across reloads and defaults it to stacked", async () => {
    ...
    testWindow.localStorage.setItem("backplane:client-settings:v1", JSON.stringify({}));
