...
const THEME_STORAGE_KEY = "backplane:theme";
...
describe("index.html boot script", () => {
  const parityCases: ReadonlyArray<{
    ...
    {
      name: "Backplane Chat follows a dark OS",
      storage: { [THEME_STORAGE_KEY]: "backplane-chat", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "an explicit global dark mode applies to Backplane Chat",
      storage: {
        [THEME_STORAGE_KEY]: "backplane-chat",
    ...
    {
      name: "a legacy backplane-grove preference resolves through the alias",
      storage: {
        [THEME_STORAGE_KEY]: "backplane-grove",
        [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true",
      },
      prefersDark: true,
    },
    {
      name: "legacy backplane-chat-dark resolves to dark Backplane Chat",
      storage: { [THEME_STORAGE_KEY]: "backplane-chat-dark" },
      prefersDark: true,
    },
    {
  ...
  it("marks built-in and custom themes on the document element", () => {
    ...
    expect(chat.themeId).toBe("backplane-chat");
  ...
  it("applies the matching half of an automatic mix to the splash", () => {
    const storage = {
      ...
      "backplane:theme-halves:v1": JSON.stringify({ dark: GROVE_THEME.id }),
    };
    ...
    expect(light.themeId).toBe("backplane-chat");
    expect(light.bootVariables["--boot-background"]).toBe(
      getThemeColorsForMode(BACKPLANE_CHAT_THEME, "light")!.canvas,
    );
  });
  ...
  it("resolves a legacy-prefixed mix half onto the renamed theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "backplane-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "backplane:theme-halves:v1": JSON.stringify({ dark: "backplane-grove" }),
      },
      prefersDark: true,
    });
    expect(boot.isDark).toBe(true);
  ...
  it("ignores a mix half that names an unknown theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "backplane-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "backplane:theme-halves:v1": JSON.stringify({ dark: "gone-theme" }),
      },
      prefersDark: true,
    });
    expect(boot.themeId).toBe("backplane-chat");
    expect(boot.isDark).toBe(true);
  });
