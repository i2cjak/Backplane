...
import { resolveWorkEntryToolPresentation } from "@backplane/client-runtime/work-log/presentation";
  ...
  it("preserves MCP server, tool, arguments, and results for expanded display", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "mcp-tool-done",
        kind: "tool.completed",
        summary: "backplane · preview_status",
        payload: {
          itemType: "mcp_tool_call",
          title: "backplane · preview_status",
          toolSurface: "browser",
    ...
    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.toolTitle).toBe("backplane · preview_status");
  ...
  it("keeps MCP payloads while collapsing lifecycle updates", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        ...
        kind: "tool.updated",
        summary: "backplane · preview_snapshot",
        payload: {
          itemType: "mcp_tool_call",
          toolCallId: "call-1",
          toolSurface: "browser",
