...
describe("deriveProviderModelsForDisplay", () => {
  ...
  it("surfaces a failed probe message in both the list row and the editor", () => {
    ...
    const driver = ProviderDriverKind.make("codex");
    const message =
      "Codex app-server provider probe failed: Cannot create Codex shadow home entry 'auth.json' because '/home/me/.codex-backplane/work/auth.json' already exists and is not a symlink.";
    const liveProvider: ServerProvider = {
      instanceId,
      driver,
      enabled: true,
