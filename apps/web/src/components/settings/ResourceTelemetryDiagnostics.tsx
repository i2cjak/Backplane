...
function categoryLabel(category: ResourceTelemetryProcessCategory): string {
  switch (category) {
    ...
    case "resource-monitor":
      return "Monitor";
    case "unknown-backplane":
      return "Backplane process";
  }
}
...
export function ResourceTelemetryDiagnostics() {
  ...
  const allBackplane = snapshot?.groups.allBackplane;
        ...
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03),0_8px_30px_rgb(0_0_0/0.035)]">
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-border/55 md:grid-cols-3">
            <IconStat
              ...
              label="Current CPU"
              value={allBackplane ? `${allBackplane.currentCpuPercent.toFixed(1)}%` : "..."}
              detail={
                allBackplane
                  ? `${formatCpuTime(allBackplane.cpuTimeMs)} observed CPU time`
                  : undefined
              }
            />
            <IconStat
              ...
              value={allBackplane ? formatBytes(allBackplane.currentRssBytes) : "..."}
              detail={
                allBackplane
                  ? `${formatBytes(allBackplane.peakRssBytes)} combined process peaks`
                  : undefined
              }
            />
            <IconStat
              ...
              label="Process count"
              value={allBackplane ? String(allBackplane.processCount) : "..."}
              detail={
                allBackplane
                  ? `${allBackplane.processStarts} starts · ${allBackplane.processExits} exits`
                  : undefined
              }
            />
            <IconStat
              ...
              value={allBackplane ? formatRate(allBackplane.ioReadBytesPerSecond) : "..."}
              detail={
                allBackplane ? `${formatBytes(allBackplane.ioReadBytes)} observed` : undefined
              }
            />
            <IconStat
              icon={<DatabaseIcon className="size-3.5" />}
              label="Write throughput"
              value={allBackplane ? formatRate(allBackplane.ioWriteBytesPerSecond) : "..."}
              detail={
                allBackplane ? `${formatBytes(allBackplane.ioWriteBytes)} observed` : undefined
              }
              tone={
                allBackplane && allBackplane.ioWriteBytesPerSecond >= 10 * 1_024 * 1_024
                  ? "danger"
                  : allBackplane && allBackplane.ioWriteBytesPerSecond >= 1_024 * 1_024
                    ? "warning"
                    : "default"
              }
            />
      ...
      >
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03)]">
          <div className="bg-muted/15 px-4 py-3 text-[11px] leading-relaxed text-muted-foreground sm:px-5">
            ...
            counters identify known Backplane operations so process spikes can be correlated with
