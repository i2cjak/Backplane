  ...
  it("can explain an unsupported environment without offering a futile retry", () => {
    const text = textOf(
      PullRequestsUnavailableState({
        ...
        error: "Update this environment's Backplane server to browse pull requests.",
      }),
    );

    expect(text).toContain("Pull requests unavailable");
    expect(text).toContain("Update this environment's Backplane server");
  ...
  it("retains the retry for transient load failures", () => {
    const html = renderToStaticMarkup(
      <PullRequestsUnavailableState
        ...
        onRetry={() => {}}
        gitHubUrl="https://github.com/i2cjak/backplane/pull/42"
      />,
    );

    expect(html).toContain("Retry");
    expect(html).toContain("Open on GitHub");
    expect(html).toContain('href="https://github.com/i2cjak/backplane/pull/42"');
  ...
  it("can offer the browser without offering a retry", () => {
    const html = renderToStaticMarkup(
      <PullRequestsUnavailableState
        ...
        gitHubUrl="https://github.com/i2cjak/backplane/pull/9"
      />,
    );
