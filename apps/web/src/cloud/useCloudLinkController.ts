...
/**
 * Drives the primary environment's Backplane Connect link. Backplane Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Backplane Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[backplane-connect] Could not update Backplane Connect", {
      message,
      traceId,
      cause,
    });
    ...
    toastManager.add({
      type: "error",
      title: "Could not update Backplane Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Backplane Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Backplane Connect before enabling this."));
        return false;
      }
