...
const ROUTER_HOST = "app.backplane.works";
const HOSTED_WEB_CHANNEL_COOKIE = "backplane_web_channel";
const LATEST_ORIGIN = "https://latest.app.backplane.works";
const NIGHTLY_ORIGIN = "https://nightly.app.backplane.works";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @backplane/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --ignore-scripts --filter '@backplane/scripts...' --filter '@backplane/web...'",
  routes: [
    {
      src: "/__backplane/channel",
