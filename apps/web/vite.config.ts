...
import { DEV_PROXIED_PATH_PREFIXES } from "@backplane/shared/devProxy";
...
const isSingleOriginDev = process.env.BACKPLANE_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_BACKPLANE_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.BACKPLANE_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: BACKPLANE_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); BACKPLANE_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.BACKPLANE_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // BACKPLANE_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.BACKPLANE_PORT, configuredWsUrl);
...
function devCompressionPlugin(): Plugin {
  return {
    name: "backplane:dev-compression",
    apply: "serve",
...
const configuredAllowedHosts = (process.env.BACKPLANE_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_BACKPLANE_RELAY_URL": JSON.stringify(configuredRelayUrl),
