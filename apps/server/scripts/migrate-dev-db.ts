 ...
 * ~/.backplane database, then run this checkout's migrations against it.
 *
 * `vp run migrate-dev-db` from a worktree:
 *   1. Nukes `<worktree>/.backplane/userdata/state.sqlite`.
...
import * as NodeSqliteClient from "@backplane/shared/nodeSqliteClient";
...
export class MigrateDevDbNotInWorktreeError extends Schema.TaggedErrorClass<MigrateDevDbNotInWorktreeError>()(
  ...
  override get message(): string {
    return "Not inside a linked git worktree. Pass --base-dir to target an isolated .backplane directory.";
  }
}
...
export class MigrateDevDbSharedHomeError extends Schema.TaggedErrorClass<MigrateDevDbSharedHomeError>()(
  ...
  override get message(): string {
    return "Refusing to rebuild the shared ~/.backplane database. Use an isolated --base-dir.";
  }
}
...
export class MigrateDevDbServerRunningError extends Schema.TaggedErrorClass<MigrateDevDbServerRunningError>()(
  ...
  override get message(): string {
    return `Dev database at '${this.databasePath}' is open by a running server (pid ${this.pid} per server-runtime.json). Stop that server first; if that pid is not actually a Backplane server (stale descriptor, reused pid), delete the server-runtime.json next to the database and retry.`;
  }
}
...
export interface RunMigrateDevDbInput {
  /** Isolated .backplane directory. Defaults to `<worktree>/.backplane` of the cwd. */
  readonly baseDir?: string | undefined;
  /** Source database. Defaults to `~/.backplane/userdata/state.sqlite`. */
...
export const runMigrateDevDb = Effect.fn("runMigrateDevDb")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".backplane"));
  ...
  const baseDir =
    input.baseDir !== undefined
      ...
      : yield* resolveWorktreeBackplaneHome(process.cwd());
...
export const migrateDevDbCommand = Command.make(
  "migrate-dev-db",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      ...
      Flag.withDescription(
        "Isolated .backplane directory. Defaults to the current worktree's .backplane.",
      ),
    ),
    source: Flag.string("source").pipe(
      Flag.optional,
      Flag.withDescription("Source database. Defaults to ~/.backplane/userdata/state.sqlite."),
    ),
  },
  ...
  Command.withDescription(
    "Rebuild the worktree dev database from a pruned snapshot of the real ~/.backplane data, then run migrations.",
  ),
);
