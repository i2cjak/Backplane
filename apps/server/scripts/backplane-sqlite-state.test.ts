...
import { runSqliteState } from "./backplane-sqlite-state.ts";
...
it.layer(NodeServices.layer)("backplane-sqlite-state", (it) => {
  it.effect("reports each invalid SQL source with a specific error", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "backplane-sqlite-state-input-",
      });
  ...
  it.effect("queries an isolated database through Effect SQL", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "backplane-sqlite-state-query-",
      });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const baseDir = yield* fs.makeTempDirectoryScoped({
          prefix: "backplane-sqlite-state-exec-",
        });
        yield* createFixtureDatabase(baseDir);
        ...
        const aliasParent = yield* fs.makeTempDirectoryScoped({
          prefix: "backplane-sqlite-state-alias-",
        });
