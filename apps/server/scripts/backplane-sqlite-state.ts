...
import { fromJsonStringPretty } from "@backplane/shared/schemaJson";
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Backplane once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedErrorClass<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.backplane database. Use an isolated --base-dir.";
  }
}
...
export const backplaneSqliteStateCommand = Command.make(
  "backplane-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Backplane base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Backplane SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(backplaneSqliteStateCommand, { version: "0.0.0" }).pipe(
