...
export default mergeConfig(
  ...
  defineConfig({
    ...
    pack: {
      ...
      define: {
        __BACKPLANE_BUILD_CHANNEL__: JSON.stringify(cliBuildChannel),
        __BACKPLANE_BUILD_RELAY_URL__: JSON.stringify(repoEnv.BACKPLANE_RELAY_URL?.trim() ?? ""),
        __BACKPLANE_BUILD_CLERK_PUBLISHABLE_KEY__: JSON.stringify(
          repoEnv.BACKPLANE_CLERK_PUBLISHABLE_KEY?.trim() ?? "",
        ),
        __BACKPLANE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: JSON.stringify(
          repoEnv.BACKPLANE_CLERK_CLI_OAUTH_CLIENT_ID?.trim() ?? "",
        ),
        __BACKPLANE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: JSON.stringify(
          repoEnv.BACKPLANE_RELAY_CLIENT_OTLP_TRACES_URL?.trim() ?? "",
        ),
        __BACKPLANE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: JSON.stringify(
          repoEnv.BACKPLANE_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() ?? "",
        ),
        __BACKPLANE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: JSON.stringify(
          repoEnv.BACKPLANE_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() ?? "",
        ),
      },
    },
    test: {
