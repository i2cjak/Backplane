...
import { HostProcessArguments } from "@backplane/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `backplane` command.
 */
...
/**
 * The `backplane` package spec to suggest. The literal spec the user typed (e.g.
 * `@backplane/cli@nightly`) is resolved away before our process starts, so re-derive it
...
function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "@backplane/cli@nightly" : "@backplane/cli";
}

/**
 * Render a `backplane <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx @backplane/cli connect` suggests
 * `npx @backplane/cli serve`, a global install suggests `backplane serve`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `backplane ${input.subcommand}`;
  }
