...
const status = {
  ...
  installed: true,
  current: true,
  unitPath: "/home/me/.config/systemd/user/backplane.service",
  logPath: "/home/me/.backplane/userdata/logs/boot-service.log",
} as const;
...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "Backplane service",
      "  Status: installed · @backplane/cli@0.0.29",
      "  Unit: /home/me/.config/systemd/user/backplane.service",
      "  Logs: /home/me/.backplane/userdata/logs/boot-service.log",
    ].join("\n"),
  );
});
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `npx @backplane/cli@0.0.29 service update`.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("npx @backplane/cli@0.0.32-nightly.1 service update");
  expect(output).not.toContain("@backplane/cli@latest");
});
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx @backplane/cli@0.0.29 service update");
  expect(output).not.toContain("npx @backplane/cli@0.0.28 service update");
});
...
it("reports a newer installed service and gives an exact-version repair command", () => {
  ...
  assert.include(
    output,
    "@backplane/cli@0.0.32-nightly.1 (newer than this @backplane/cli@0.0.31 CLI)",
  );
  assert.include(output, "npx @backplane/cli@0.0.32-nightly.1 service update");
  assert.notInclude(output, "npx @backplane/cli@latest service update");
});
...
it.layer(Layer.mergeAll(NodeServices.layer, NetService.layer))("service commands", (it) => {
    ...
    (command) =>
      Effect.gen(function* () {
        ...
        const baseDir = yield* fs.makeTempDirectoryScoped({
          prefix: "backplane-service-cli-test-",
        });
  ...
  it.effect.each(["install", "update"] as const)("%s allows an explicit downgrade", (command) =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "backplane-service-cli-test-" });
      const { service, installOptions } = makeTestService(newerServiceStatus);
      vi.spyOn(BootService, "layer").mockReturnValue(
        Layer.succeed(BootService.BootService, service),
      );
