...
const makeBaseDir = () =>
  NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "backplane-theme-cli-"));
...
describe("backplane theme", () => {
  ...
  it.effect("publishes a theme file under an explicit id", () =>
    Effect.gen(function* () {
      const baseDir = makeBaseDir();
      const themeFile = NodePath.join(baseDir, "backplane.json");
      NodeFS.writeFileSync(themeFile, NIGHTFALL_THEME_JSON);
  ...
  it.effect("honors BACKPLANE_HOME like the rest of the CLI", () =>
    Effect.gen(function* () {
      const baseDir = makeBaseDir();
      yield* runCli(["theme", "set", "ocean"]).pipe(
        Effect.provide(
          ConfigProvider.layer(ConfigProvider.fromEnv({ env: { BACKPLANE_HOME: baseDir } })),
        ),
      );
      assert.equal(readSettings(baseDir).defaultTheme, "ocean");
    }),
  ...
  it.effect("rejects the mobile default theme id", () =>
    Effect.gen(function* () {
      const baseDir = makeBaseDir();
      const failure = yield* runCli(["theme", "set", "backplane", "--base-dir", baseDir]).pipe(
        Effect.flip,
      );
      assert.include(String(failure), "No theme named");
    }),
