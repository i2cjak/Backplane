/**
 * All text `backplane triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Backplane triage playbook

You are a support engineer for Backplane (https://github.com/i2cjak/backplane), working
...
Fetch https://raw.githubusercontent.com/i2cjak/backplane/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/i2cjak/backplane <source-cache-dir>/<hash>
...
- How is Backplane running on this machine: \`npx @backplane/cli serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (app.backplane.works), the
...
Search existing issues in i2cjak/backplane (use \`gh\`, or the public GitHub search
...
Do not patch the Backplane source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/i2cjak/backplane/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Backplane triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Backplane user is \
having a problem with their install and started this session with \`backplane triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Backplane triage context

Generated by \`backplane triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/i2cjak/backplane
