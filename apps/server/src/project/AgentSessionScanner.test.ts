    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        ServerConfig.layerTest(
          ...
          input.configBaseDir ?? { prefix: "backplane-scanner-config-" },
        ),
...
it.layer(NodeServices.layer)("AgentSessionScanner", (it) => {
  describe("scan", () => {
    it.effect("reads Claude project cwds from transcripts, newest first", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("backplane-claude-home-");
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
        const olderWorkspace = yield* makeTempDir("backplane-workspace-older-");
        const newerWorkspace = yield* makeTempDir("backplane-workspace-newer-");
    ...
    it.effect("groups Codex rollouts by cwd across date directories", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("backplane-claude-home-");
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
        const workspace = yield* makeTempDir("backplane-workspace-");
        const otherWorkspace = yield* makeTempDir("backplane-workspace-other-");
    ...
    it.effect("returns the imported project ID through a realpath alias", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
        const workspace = yield* makeTempDir("backplane-workspace-");
        const linkParent = yield* makeTempDir("backplane-scanner-links-");
    ...
    it.effect("keeps case variants distinct when the filesystem identities differ", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("backplane-claude-home-");
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
        const backingUpper = yield* makeTempDir("backplane-backing-upper-");
        const backingLower = yield* makeTempDir("backplane-backing-lower-");
        const aliasParent = yield* makeTempDir("backplane-case-aliases-");
    ...
    it.effect("uses explicit provider instance homes instead of overridden legacy homes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("backplane-claude-legacy-");
        const codexHomePath = yield* makeTempDir("backplane-codex-legacy-");
        const claudeInstanceHome = yield* makeTempDir("backplane-claude-instance-");
        const codexInstanceHome = yield* makeTempDir("backplane-codex-instance-");
        const legacyWorkspace = yield* makeTempDir("backplane-workspace-legacy-");
        const claudeWorkspace = yield* makeTempDir("backplane-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("backplane-workspace-codex-");
    ...
    it.effect("scans each distinct home across multiple instances once", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("backplane-claude-home-");
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
        const otherCodexHome = yield* makeTempDir("backplane-codex-other-");
        const workspace = yield* makeTempDir("backplane-workspace-");
        const otherWorkspace = yield* makeTempDir("backplane-workspace-other-");
    ...
    it.effect("honors provider instance home directory environment variables", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("backplane-claude-legacy-");
        const codexHomePath = yield* makeTempDir("backplane-codex-legacy-");
        const claudeEnvironmentHome = yield* makeTempDir("backplane-claude-env-");
        const codexEnvironmentHome = yield* makeTempDir("backplane-codex-env-");
        const claudeWorkspace = yield* makeTempDir("backplane-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("backplane-workspace-codex-");
    ...
    it.effect("does not scan provider instances disabled by the envelope or config", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("backplane-claude-home-");
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
        const envelopeDisabledHome = yield* makeTempDir("backplane-codex-disabled-envelope-");
        const configDisabledHome = yield* makeTempDir("backplane-codex-disabled-config-");
        const envelopeWorkspace = yield* makeTempDir("backplane-workspace-disabled-envelope-");
        const configWorkspace = yield* makeTempDir("backplane-workspace-disabled-config-");
    ...
    it.effect("excludes the home directory, temporary root, and Backplane data directory", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("backplane-claude-home-");
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
        const configBaseDir = yield* makeTempDir("backplane-scanner-base-");
        const workspace = yield* makeTempDir("backplane-workspace-");
    ...
    it.effect("excludes Backplane-managed worktree sandboxes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("backplane-claude-home-");
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
    ...
    it.effect(
      "excludes sandboxes under the configured worktrees dir without .backplane in the path",
      () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const claudeHomePath = yield* makeTempDir("backplane-claude-home-");
          ...
          // dir contains no `.backplane` segment — only the config-based prefix match
          // can exclude this one.
          const worktreeCwd = path.join(configBaseDir, "worktrees", "backplane", "wt-2");
          yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
          yield* writeTranscript({
            filePath: path.join(claudeHomePath, "projects", "-slug", "a.jsonl"),
            contents: claudeSessionLine(worktreeCwd),
    ...
    it.effect("excludes sandboxes reached through a symlink into the worktrees dir", () =>
      Effect.gen(function* () {
        ...
        const worktreeCwd = path.join(configBaseDir, "worktrees", "backplane", "wt-3");
        yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
    ...
    it.effect.each([64, 65])("shares metadata bytes across homes for %s one-MiB files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("backplane-metadata-home-");
        const secondHome = yield* makeTempDir("backplane-metadata-second-");
        const codexHomePath = yield* makeTempDir("backplane-metadata-codex-");
        const firstWorkspace = yield* makeTempDir("backplane-metadata-first-project-");
        const secondWorkspace = yield* makeTempDir("backplane-metadata-second-project-");
        const directories = [
    ...
    it.effect.each([50, 51])("bounds metadata open/read calls for %s short-read files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("backplane-short-metadata-home-");
        const codexHomePath = yield* makeTempDir("backplane-short-metadata-codex-");
        const workspace = yield* makeTempDir("backplane-short-metadata-project-");
    ...
    it.effect("bounds malformed metadata records without excluding another account", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("backplane-record-metadata-home-");
        const secondHome = yield* makeTempDir("backplane-record-metadata-second-");
        const codexHomePath = yield* makeTempDir("backplane-record-metadata-codex-");
        const workspace = yield* makeTempDir("backplane-record-metadata-project-");
        const directory = path.join(claudeHomePath, "projects", "p");
        const template = path.join(directory, "template.jsonl");
        yield* writeTranscript({
          filePath: template,
    ...
    it.effect.each([19_999, 20_000])(
      ...
      (count) =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const fileSystem = yield* FileSystem.FileSystem;
          const claudeHomePath = yield* makeTempDir("backplane-directory-budget-home-");
          const codexHomePath = yield* makeTempDir("backplane-directory-budget-codex-");
    ...
    it.effect("returns an empty result when neither home directory exists", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const root = yield* makeTempDir("backplane-missing-homes-");
  ...
  describe("recentThreads", () => {
    it.effect.each([false, true])(
      ...
      (overflow) =>
        Effect.gen(function* () {
          ...
          const claudeHomePath = yield* makeTempDir("backplane-record-limit-claude-");
          const codexHomePath = yield* makeTempDir("backplane-record-limit-codex-");
          const workspace = yield* makeTempDir("backplane-record-limit-project-");
    ...
    it.effect("keeps the provider instance that owns a custom session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("backplane-claude-home-");
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
        const customHome = yield* makeTempDir("backplane-codex-custom-");
        const workspace = yield* makeTempDir("backplane-workspace-");
    ...
    it.effect("shares a 64 MiB full-read budget across providers without hiding projects", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("backplane-budget-claude-");
        const codexHomePath = yield* makeTempDir("backplane-budget-codex-");
        const workspace = yield* makeTempDir("backplane-budget-workspace-");
    ...
    it.effect("skips excessive records without blocking an older valid transcript", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("backplane-record-budget-claude-");
        const codexHomePath = yield* makeTempDir("backplane-record-budget-codex-");
        const workspace = yield* makeTempDir("backplane-record-budget-workspace-");
        ...
        it.effect.skipIf(replacement === "symlink alias" && !symlinksSupported)(
          ...
          () =>
            Effect.gen(function* () {
              ...
              const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
              yield* TestClock.setTime(nowMs);
              const fixture = yield* makeTempDir("backplane-replaced-cwd-");
    ...
    it.effect("checks file identity and provider before skipping completed history", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("backplane-completed-claude-");
        const codexHomePath = yield* makeTempDir("backplane-completed-codex-");
        const workspace = yield* makeTempDir("backplane-completed-workspace-");
    ...
    it.effect("does not reopen a transcript that becomes a non-file after discovery", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
        const workspace = yield* makeTempDir("backplane-workspace-");
        const nonFilePath = yield* makeTempDir("backplane-non-file-");
    ...
    it.effect("does not import sessions from a Backplane-managed worktree", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        ...
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
        const configBaseDir = yield* makeTempDir("backplane-scanner-base-");
        const workspace = path.join(configBaseDir, "worktrees", "backplane", "managed-worktree");
    ...
    it.effect("uses one deterministic provider instance for a shared session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("backplane-claude-home-");
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
        const sharedHome = yield* makeTempDir("backplane-codex-shared-");
        const workspace = yield* makeTempDir("backplane-workspace-");
    ...
    it.effect("keeps a second account when the first has 5000 newer files", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("backplane-claude-home-");
        const codexHomePath = yield* makeTempDir("backplane-codex-home-");
        const oldWorkspace = yield* makeTempDir("backplane-workspace-old-");
        const recentWorkspace = yield* makeTempDir("backplane-workspace-recent-");
        const recentHome = yield* makeTempDir("backplane-claude-recent-home-");
