...
import type { ClientOs } from "@backplane/contracts";
...
const TelemetryEnvConfig = Config.all({
  posthogKey: Config.string("BACKPLANE_POSTHOG_KEY").pipe(
    Config.withDefault("phc_XOWci4oZP4VvLiEyrFqkFjP4CZn55mjYYBMREK5Wd6m"),
  ),
  posthogHost: Config.string("BACKPLANE_POSTHOG_HOST").pipe(
    Config.withDefault("https://us.i.posthog.com"),
  ),
  enabled: Config.boolean("BACKPLANE_TELEMETRY_ENABLED").pipe(Config.withDefault(true)),
  flushBatchSize: Config.number("BACKPLANE_TELEMETRY_FLUSH_BATCH_SIZE").pipe(
    Config.withDefault(20),
  ),
  maxBufferedEvents: Config.number("BACKPLANE_TELEMETRY_MAX_BUFFERED_EVENTS").pipe(
...
>()("@backplane/cli/telemetry/AnalyticsService") {
    ...
    const payload = {
      ...
      batch: events.map((event) => ({
        ...
        properties: {
          ...
          backplaneCodeVersion: packageJson.version,
          clientType,
