...
export interface TelemetryCounters {
  ...
  readonly allBackplane: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allBackplane: ResourceTelemetryAggregate;
  };
...
export const emptyTelemetryCounters = (): TelemetryCounters => ({
  ...
  allBackplane: emptyGroupCounters(),
});
...
function applyLifecycleCounters(input: {
  ...
  let allBackplane = input.counters.allBackplane;
  for (const processDelta of input.deltas) {
    ...
    allBackplane = incrementCounters(allBackplane, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allBackplane = incrementCounters(allBackplane, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allBackplane = incrementCounters(allBackplane, { processExits: 1 });
  }

  return { backend, electron, monitor, allBackplane };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allBackplane: aggregate(ordered, counters.allBackplane),
    },
    deltas: processDeltas,
  };
}
