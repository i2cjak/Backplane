...
} from "@backplane/shared/connectAuth";
import { clerkFrontendApiUrlFromPublishableKey } from "@backplane/shared/relayAuth";
import { normalizeSecureRelayUrl } from "@backplane/shared/relayUrl";
...
declare const __BACKPLANE_BUILD_RELAY_URL__: string | undefined;
declare const __BACKPLANE_BUILD_CLERK_PUBLISHABLE_KEY__: string | undefined;
declare const __BACKPLANE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: string | undefined;
declare const __BACKPLANE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: string | undefined;
declare const __BACKPLANE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: string | undefined;
declare const __BACKPLANE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: string | undefined;
...
export const buildTimeRelayUrl =
  typeof __BACKPLANE_BUILD_RELAY_URL__ === "undefined"
    ? ""
    : (normalizeSecureRelayUrl(__BACKPLANE_BUILD_RELAY_URL__) ?? "");
export const buildTimeClerkPublishableKey = readBuildTimeValue(
  typeof __BACKPLANE_BUILD_CLERK_PUBLISHABLE_KEY__ === "undefined"
    ? undefined
    : __BACKPLANE_BUILD_CLERK_PUBLISHABLE_KEY__,
);
export const buildTimeClerkCliOAuthClientId = readBuildTimeValue(
  typeof __BACKPLANE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__ === "undefined"
    ? undefined
    : __BACKPLANE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__,
);
export const buildTimeRelayClientTracing = {
  tracesUrl: readBuildTimeValue(
    typeof __BACKPLANE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__ === "undefined"
      ? undefined
      : __BACKPLANE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__,
  ),
  tracesDataset: readBuildTimeValue(
    typeof __BACKPLANE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__ === "undefined"
      ? undefined
      : __BACKPLANE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__,
  ),
  tracesToken: readBuildTimeValue(
    typeof __BACKPLANE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__ === "undefined"
      ? undefined
      : __BACKPLANE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__,
  ),
} as const;
...
export function resolveRelayClientTracingConfig(
  ...
  const tracesUrl = env.BACKPLANE_RELAY_CLIENT_OTLP_TRACES_URL?.trim() || fallback.tracesUrl;
  const tracesDataset =
    env.BACKPLANE_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() || fallback.tracesDataset;
  const tracesToken = env.BACKPLANE_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() || fallback.tracesToken;
...
export function makeRelayUrlConfig(fallback = buildTimeRelayUrl) {
  const runtimeConfig = Config.nonEmptyString("BACKPLANE_RELAY_URL");
...
export function makeCloudCliOAuthConfig({
  ...
  return Config.all({
    clerkPublishableKey: makePublicValueConfig(
      "BACKPLANE_CLERK_PUBLISHABLE_KEY",
      clerkPublishableKeyFallback,
    ),
    clientId: makePublicValueConfig(
      "BACKPLANE_CLERK_CLI_OAUTH_CLIENT_ID",
...
export const hasCloudPublicConfig = Boolean(
  (normalizeSecureRelayUrl(process.env.BACKPLANE_RELAY_URL ?? "") ?? buildTimeRelayUrl) &&
  (process.env.BACKPLANE_CLERK_PUBLISHABLE_KEY?.trim() || buildTimeClerkPublishableKey) &&
  (process.env.BACKPLANE_CLERK_CLI_OAUTH_CLIENT_ID?.trim() || buildTimeClerkCliOAuthClientId),
);
