  ...
  ProcessRunner.ProcessRunner.of({
    run: (input) =>
      Effect.gen(function* () {
        ...
        const entry = path.join(stagingDir, "node_modules", "backplane", "dist", "bin.mjs");
...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("installs through pnpm when its Node runtime has no npm executable", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "backplane-pinned-pnpm-" });
  ...
  it.effect("does not try a different installer for npm permission failures", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "backplane-pinned-permission-" });
  ...
  it.effect("validates a staging tree before atomically publishing it", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "backplane-pinned-runtime-test-",
      });
  ...
  it.effect("replaces an incomplete pinned runtime", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "backplane-pinned-runtime-repair-",
      });
      const finalPaths = pinnedRuntimePaths(path, baseDir, "1.2.3");
  ...
  it.effect("removes staging when installation is interrupted", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "backplane-pinned-runtime-interrupt-",
      });
