...
import { ServerSelfUpdateError, ThreadId } from "@backplane/contracts";
...
const makeHarness = Effect.fn("test.make_self_update_harness")(function* (
  ...
  const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "backplane-self-update-test-" });
  const order: string[] = [];
    ...
    run: (input) =>
      Effect.gen(function* () {
        if (input.command === "npm") {
          ...
          const entry = path.join(prefix, "node_modules", "backplane", "dist", "bin.mjs");
...
it.layer(NodeServices.layer)("server self update", (it) => {
  ...
  it.effect("rejects invalid versions and desktop-managed servers before staging", () =>
    Effect.gen(function* () {
      ...
      ).toBe("'latest' is not an exact backplane version.");
