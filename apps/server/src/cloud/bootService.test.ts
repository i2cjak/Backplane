...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    launcherPath: "/home/theo/.backplane/runtime/service-launcher.mjs",
    baseDir: "/home/theo/.backplane",
    logPath: "/home/theo/.backplane/userdata/logs/boot-service.log",
    unitPath: "/home/theo/.config/systemd/user/backplane.service",
  });

  expect(unit).toContain(
    "ExecStart=/usr/bin/node /home/theo/.backplane/runtime/service-launcher.mjs",
  );
...
const macPlan = {
  ...
  launcherPath: "/Users/theo/.backplane/runtime/service-launcher.mjs",
  baseDir: "/Users/theo/.backplane",
  logPath: "/Users/theo/.backplane/userdata/logs/boot-service.log",
  unitPath: "/Users/theo/Library/LaunchAgents/works.backplane.app.service.plist",
};
...
it("keeps launchd pinned to the stable launcher rather than a versioned server", () => {
  ...
  expect(plist).toContain("<string>/Users/theo/.backplane/runtime/service-launcher.mjs</string>");
...
it("appends both stdio streams to the boot service log", () => {
  ...
  expect(plist).toContain(
    "<key>StandardOutPath</key>\n  <string>/Users/theo/.backplane/userdata/logs/boot-service.log</string>",
  );
  expect(plist).toContain(
...
it("escapes XML in host paths", () => {
  const plist = BootService.renderBootServicePlist(
    { ...macPlan, baseDir: "/Users/theo/Backplane & <Co>" },
    { homeDir: "/Users/theo", environmentPath: "/Users/theo/Tools & <Scripts>:/usr/bin" },
  );

  expect(plist).toContain("<string>/Users/theo/Backplane &amp; &lt;Co&gt;</string>");
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable backplane.service")
        control.enabled = true;
      if (!failed && command === "systemctl --user restart backplane.service")
        control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop backplane.service" ||
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop backplane.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now backplane.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? [
                "systemctl --user stop backplane.service",
                "systemctl --user restart backplane.service",
              ]
            : [
                "launchctl bootout --wait gui/501/works.backplane.app.service",
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "works.backplane.app.service.plist"),
        ),
      ).toBe(true);
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/works.backplane.app.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/works.backplane.app.service",
        "launchctl enable gui/501/works.backplane.app.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/works.backplane.app.service";
