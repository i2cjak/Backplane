...
it.effect("prefers a runtime relay URL override over the statically injected value", () =>
  Effect.gen(function* () {
    const relayUrl = yield* makeRelayUrlConfig("https://embedded.example.test").pipe(
      provideEnv({ BACKPLANE_RELAY_URL: "https://runtime.example.test///" }),
    );
...
it.effect("rejects an insecure runtime relay URL override", () =>
  makeRelayUrlConfig("https://embedded.example.test").pipe(
    provideEnv({ BACKPLANE_RELAY_URL: "http://runtime.example.test" }),
    Effect.flip,
  ),
);
  ...
  Effect.gen(function* () {
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ BACKPLANE_HOSTED_APP_URL: "https://nightly.app.backplane.works" }),
      ),
      "https://nightly.app.backplane.works",
    );
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ BACKPLANE_HOSTED_APP_URL: "http://localhost:5733" }),
      ),
      "http://localhost:5733",
    );
  }),
...
it.effect("rejects malformed or insecure hosted app URLs", () =>
  Effect.gen(function* () {
    for (const value of [
      "app.backplane.works",
      "http://app.backplane.works",
      "https://app.backplane.works/nested",
      "https://app.backplane.works?alias=true",
    ]) {
      const result = yield* hostedAppUrlConfig.pipe(
        provideEnv({ BACKPLANE_HOSTED_APP_URL: value }),
        Effect.result,
      );
...
it.effect("prefers runtime Clerk OAuth config overrides over statically injected values", () =>
  Effect.gen(function* () {
    const config = yield* makeCloudCliOAuthConfig({
      ...
      provideEnv({
        BACKPLANE_CLERK_PUBLISHABLE_KEY: "pk_test_cnVudGltZS5leGFtcGxlLnRlc3Qk",
        BACKPLANE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_client_runtime",
      }),
    );
...
it("resolves relay client tracing from runtime config with build-time fallback", () => {
  ...
  assert.deepEqual(
    resolveRelayClientTracingConfig(
      {
        BACKPLANE_RELAY_CLIENT_OTLP_TRACES_URL: "https://runtime.example.test/v1/traces",
        BACKPLANE_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        BACKPLANE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
    ),
    {
  ...
  assert.equal(
    resolveRelayClientTracingConfig(
      {
        BACKPLANE_RELAY_CLIENT_OTLP_TRACES_URL: "http://insecure.example.test/v1/traces",
        BACKPLANE_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        BACKPLANE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
