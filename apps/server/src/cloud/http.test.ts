...
import { RelayClientTracer } from "@backplane/shared/relayTracing";
...
import type { RelayLinkProofRequest } from "@backplane/contracts/relay";
  ...
  it.effect("does not accept an unauthenticated request trace parent", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("https://environment.example.test/api/backplane-cloud/mint-credential", {
          headers: {
...
describe("releaseManagedTunnelOnShutdown", () => {
  ...
  const provideReleaseHarness =
    ...
    <A, E, R>(effect: Effect.Effect<A, E, R>) =>
      effect.pipe(
        ...
        Effect.provide(
          ServerConfigModule.layerTest("/", { prefix: "backplane-http-release-test-" }).pipe(
  ...
  it.effect("still releases a pending update when the launcher is stopping", () => {
    // `backplane service uninstall` or `systemctl stop` during the pending window:
...
describe("link proof provider kinds", () => {
  ...
  it("accepts managed and manual endpoints but not backplane_relay", () => {
    ...
    expect(isSupportedLinkProviderKind(proofRequest("backplane_relay"))).toBe(false);
  });
