...
/**
 * A pinned runtime is an exact `@backplane/cli@<version>` npm-installed into
 ...
 * switching over, never `npx backplane`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "backplane", "dist", "bin.mjs"),
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const stagingPaths: PinnedRuntimePaths = {
    ...
    entryPath: input.path.join(stagingDir, "node_modules", "backplane", "dist", "bin.mjs"),
