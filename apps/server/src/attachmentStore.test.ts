...
describe("attachmentStore", () => {
  ...
  it("resolves attachment path by id using the extension that exists on disk", () => {
    const attachmentsDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "backplane-attachment-store-"),
    );
    try {
  ...
  it("resolves generic attachments without scanning the attachment directory", () => {
    const attachmentsDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "backplane-file-attachment-"),
    );
    try {
  ...
  it("plans pending attachment claims with direct filename lookups", () => {
    const attachmentsDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "backplane-attachment-claim-"),
    );
    try {
  ...
  it("rejects thread-owned attachments even when thread segments collide", () => {
    const attachmentsDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "backplane-attachment-ownership-"),
    );
    try {
  ...
  it("removes expired pending and partial files without touching thread attachments", () => {
    const attachmentsDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "backplane-attachment-sweep-"),
    );
    try {
