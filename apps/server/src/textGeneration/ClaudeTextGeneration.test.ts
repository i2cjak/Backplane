...
import { ClaudeSettings, ProviderInstanceId } from "@backplane/contracts";
import { HostProcessPlatform, isHostWindows } from "@backplane/shared/hostProcess";
...
const ClaudeTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "backplane-claude-text-generation-test-",
  ...
  return Effect.gen(function* () {
    ...
    writeFakeCli({
      ...
      source: [
        ...
        "}",
        "",
        "const argsMustContain = process.env.BACKPLANE_FAKE_CLAUDE_ARGS_MUST_CONTAIN;",
        "if (argsMustContain && !args.includes(argsMustContain)) {",
        '  fail("args missing expected content", 2);',
        "}",
        "",
        "const argsMustNotContain = process.env.BACKPLANE_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN;",
        "if (argsMustNotContain && args.includes(argsMustNotContain)) {",
        '  fail("args contained forbidden content", 3);',
        "}",
        "",
        "const stdinMustContain = process.env.BACKPLANE_FAKE_CLAUDE_STDIN_MUST_CONTAIN;",
        "if (stdinMustContain && !stdinContent.includes(stdinMustContain)) {",
        '  fail("stdin missing expected content", 4);',
        "}",
        "",
        "const configDirMustBe = process.env.BACKPLANE_FAKE_CLAUDE_CONFIG_DIR_MUST_BE;",
        ...
        "}",
        "",
        "const stderrText = process.env.BACKPLANE_FAKE_CLAUDE_STDERR;",
        "if (stderrText) {",
        '  process.stderr.write(stderrText + "\\n");',
        "}",
        "",
        'process.stdout.write(process.env.BACKPLANE_FAKE_CLAUDE_OUTPUT ?? "");',
        "process.exitCode = Number(process.env.BACKPLANE_FAKE_CLAUDE_EXIT_CODE ?? 0);",
        "",
      ].join("\n"),
    });
    return binDir;
...
function withFakeClaudeEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    ...
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "backplane-claude-text-" });
    const binDir = yield* makeFakeClaudeBinary(tempDir);
    const pathDelimiter = (yield* isHostWindows) ? ";" : ":";
    const previousPath = process.env.PATH;
    const previousOutput = process.env.BACKPLANE_FAKE_CLAUDE_OUTPUT;
    const previousExitCode = process.env.BACKPLANE_FAKE_CLAUDE_EXIT_CODE;
    const previousStderr = process.env.BACKPLANE_FAKE_CLAUDE_STDERR;
    const previousArgsMustContain = process.env.BACKPLANE_FAKE_CLAUDE_ARGS_MUST_CONTAIN;
    const previousArgsMustNotContain = process.env.BACKPLANE_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN;
    const previousStdinMustContain = process.env.BACKPLANE_FAKE_CLAUDE_STDIN_MUST_CONTAIN;
    const previousConfigDirMustBe = process.env.BACKPLANE_FAKE_CLAUDE_CONFIG_DIR_MUST_BE;

    yield* Effect.acquireRelease(
      Effect.sync(() => {
        process.env.PATH = `${binDir}${pathDelimiter}${previousPath ?? ""}`;
        process.env.BACKPLANE_FAKE_CLAUDE_OUTPUT = input.output;

        if (input.exitCode !== undefined) {
          process.env.BACKPLANE_FAKE_CLAUDE_EXIT_CODE = String(input.exitCode);
        } else {
          delete process.env.BACKPLANE_FAKE_CLAUDE_EXIT_CODE;
        }

        if (input.stderr !== undefined) {
          process.env.BACKPLANE_FAKE_CLAUDE_STDERR = input.stderr;
        } else {
          delete process.env.BACKPLANE_FAKE_CLAUDE_STDERR;
        }

        if (input.argsMustContain !== undefined) {
          process.env.BACKPLANE_FAKE_CLAUDE_ARGS_MUST_CONTAIN = input.argsMustContain;
        } else {
          delete process.env.BACKPLANE_FAKE_CLAUDE_ARGS_MUST_CONTAIN;
        }

        if (input.argsMustNotContain !== undefined) {
          process.env.BACKPLANE_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN = input.argsMustNotContain;
        } else {
          delete process.env.BACKPLANE_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN;
        }

        if (input.stdinMustContain !== undefined) {
          process.env.BACKPLANE_FAKE_CLAUDE_STDIN_MUST_CONTAIN = input.stdinMustContain;
        } else {
          delete process.env.BACKPLANE_FAKE_CLAUDE_STDIN_MUST_CONTAIN;
        }

        if (input.configDirMustBe !== undefined) {
          process.env.BACKPLANE_FAKE_CLAUDE_CONFIG_DIR_MUST_BE = input.configDirMustBe;
        } else {
          delete process.env.BACKPLANE_FAKE_CLAUDE_CONFIG_DIR_MUST_BE;
        }
      }),
      () =>
        Effect.sync(() => {
          ...
          if (previousOutput === undefined) {
            delete process.env.BACKPLANE_FAKE_CLAUDE_OUTPUT;
          } else {
            process.env.BACKPLANE_FAKE_CLAUDE_OUTPUT = previousOutput;
          }

          if (previousExitCode === undefined) {
            delete process.env.BACKPLANE_FAKE_CLAUDE_EXIT_CODE;
          } else {
            process.env.BACKPLANE_FAKE_CLAUDE_EXIT_CODE = previousExitCode;
          }

          if (previousStderr === undefined) {
            delete process.env.BACKPLANE_FAKE_CLAUDE_STDERR;
          } else {
            process.env.BACKPLANE_FAKE_CLAUDE_STDERR = previousStderr;
          }

          if (previousArgsMustContain === undefined) {
            delete process.env.BACKPLANE_FAKE_CLAUDE_ARGS_MUST_CONTAIN;
          } else {
            process.env.BACKPLANE_FAKE_CLAUDE_ARGS_MUST_CONTAIN = previousArgsMustContain;
          }

          if (previousArgsMustNotContain === undefined) {
            delete process.env.BACKPLANE_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN;
          } else {
            process.env.BACKPLANE_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN = previousArgsMustNotContain;
          }

          if (previousStdinMustContain === undefined) {
            delete process.env.BACKPLANE_FAKE_CLAUDE_STDIN_MUST_CONTAIN;
          } else {
            process.env.BACKPLANE_FAKE_CLAUDE_STDIN_MUST_CONTAIN = previousStdinMustContain;
          }

          if (previousConfigDirMustBe === undefined) {
            delete process.env.BACKPLANE_FAKE_CLAUDE_CONFIG_DIR_MUST_BE;
          } else {
            process.env.BACKPLANE_FAKE_CLAUDE_CONFIG_DIR_MUST_BE = previousConfigDirMustBe;
          }
        }),
    );
