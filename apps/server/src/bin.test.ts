...
const makeProjectLookupFixture = Effect.fn("makeProjectLookupFixture")(function* (
  ...
  const baseDir = NodeFS.mkdtempSync(
    NodePath.join(NodeOS.tmpdir(), "backplane-cli-project-lookup-state-"),
  );
  const workspaceRoot = NodeFS.mkdtempSync(
    NodePath.join(NodeOS.tmpdir(), "backplane-cli-project-lookup-git-"),
  );
  ...
  it.effect("cannot remove the old environment's ID from a replacement empty database", () =>
    Effect.gen(function* () {
      ...
      const replacementDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-project-lookup-new-state-"),
      );
      const error = yield* runCliWithRuntime([
        "project",
        "remove",
...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("rejects connect commands when public configuration is missing", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["backplane", "connect"]);
      assert.include(
        error.errors[0]?.message ?? "",
        "missing Backplane Connect public configuration",
      );

      const output = (yield* TestConsole.errorLines).join("\n");
      assert.include(output, "ERROR");
      assert.include(output, "missing Backplane Connect public configuration");
    }).pipe(Effect.provide(Layer.mergeAll(CliRuntimeLayer, TestConsole.layer))),
  );

  it.effect("exposes service lifecycle commands without Backplane Connect configuration", () =>
    Effect.gen(function* () {
      const { output } = yield* captureStdout(runCli(["service", "--help"], noConnectCli));

      assert.include(output, "Manage the Backplane background service.");
  ...
  it.effect("reports fresh headless connect state without requiring local configuration", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-cloud-status-test-"),
      );
  ...
  it.effect("reports actionable human-readable headless connect state", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-cloud-status-human-test-"),
      );
      const { output } = yield* captureStdout(
        runConnectCli(["connect", "status", "--base-dir", baseDir]),
      );

      assert.include(output, "Backplane Connect\n  Exposure: disabled");
      assert.include(output, "  Authorization: missing");
      assert.include(output, "  Environment link: not provisioned");
      assert.include(
        output,
        "Next: Run `backplane connect link` to authorize and enable Backplane Connect.",
      );
    }),
  );
  ...
  it.effect("accepts the --headless login override without enabling access", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-cloud-login-test-"),
      );
  ...
  it.effect("disables headless connect without a running server", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-cloud-unlink-test-"),
      );
      const { output } = yield* captureStdout(
        runConnectCli(["connect", "unlink", "--base-dir", baseDir]),
      );

      assert.equal(output, "Backplane Connect is disabled locally.");
    }),
  );
  ...
  it.effect("logs out of headless connect and removes the stored CLI authorization", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-cloud-logout-test-"),
      );
      const { secretsDir } = yield* ServerConfig.deriveServerPaths(baseDir, undefined);
  ...
  it.effect("executes auth pairing subcommands and redacts secrets from list output", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-auth-pairing-test-"),
      );
  ...
  it.effect("executes auth session subcommands and redacts secrets from list output", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-auth-session-test-"),
      );
  ...
  it.effect("rejects invalid ttl values before running auth commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["backplane", "auth", "pairing", "create"]);
  ...
  it.effect("adds, renames, and removes projects offline through the orchestration engine", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-projects-offline-test-"),
      );
      const workspaceRoot = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-projects-workspace-"),
      );
  ...
  it.effect("force removes projects that still contain threads", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-projects-force-remove-test-"),
      );
      const workspaceRoot = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-projects-force-remove-workspace-"),
      );
  ...
  it.effect("routes project commands through a running server when runtime state is present", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-projects-live-test-"),
      );
      const workspaceRoot = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-projects-live-workspace-"),
      );
  ...
  it.effect("rejects dev-url on project commands", () =>
    Effect.gen(function* () {
      const workspaceRoot = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "backplane-cli-projects-unknown-option-workspace-"),
      );
      const error = yield* runCliWithRuntime([
        "project",
        "add",
      ...
      assert.deepEqual(error.commandPath, ["backplane", "project", "add"]);
