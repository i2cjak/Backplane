...
import { VcsProcessExitError } from "@backplane/contracts";
  ...
  it.effect("parses merge request view output", () =>
    Effect.gen(function* () {
        ...
        Effect.succeed(
          processOutput(
            ...
            JSON.stringify({
              ...
              web_url: "https://gitlab.com/i2cjak/backplane/-/merge_requests/42",
              target_branch: "main",
              ...
              source_project: {
                path_with_namespace: "octocat/backplane",
              },
            }),
          ),
        ),
      ...
      assert.deepStrictEqual(result, {
        number: 42,
        title: "Add MR thread creation",
        url: "https://gitlab.com/i2cjak/backplane/-/merge_requests/42",
        baseRefName: "main",
            ...
            JSON.stringify([
              {
                iid: 0,
                title: "invalid",
                web_url: "https://gitlab.com/i2cjak/backplane/-/merge_requests/0",
                target_branch: "main",
              ...
              },
              {
                iid: 43,
                title: "  Valid MR  ",
                web_url: " https://gitlab.com/i2cjak/backplane/-/merge_requests/43 ",
                target_branch: " main ",
      ...
      assert.deepStrictEqual(result, [
        {
          number: 43,
          title: "Valid MR",
          url: "https://gitlab.com/i2cjak/backplane/-/merge_requests/43",
          baseRefName: "main",
  ...
  it.effect("reads repository clone URLs", () =>
    Effect.gen(function* () {
      mockedRun.mockReturnValueOnce(
        Effect.succeed(
          processOutput(
            ...
            JSON.stringify({
              path_with_namespace: "octocat/backplane",
              web_url: "https://gitlab.com/octocat/backplane",
              http_url_to_repo: "https://gitlab.com/octocat/backplane.git",
              ssh_url_to_repo: "git@gitlab.com:octocat/backplane.git",
            }),
          ),
        ),
      );
      ...
      assert.deepStrictEqual(result, {
        ...
        url: "https://gitlab.com/octocat/backplane",
        sshUrl: "git@gitlab.com:octocat/backplane.git",
      });
    }),
  );
  ...
  it.effect("creates merge requests through the GitLab API without placing the body in argv", () =>
    Effect.gen(function* () {
      ...
      yield* glab.createMergeRequest({
        ...
        title: "Provider MR",
        bodyFile: "/tmp/backplane-mr-body.md",
      });
      ...
      expect(mockedRun).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          args: [
            ...
            "title=Provider MR",
            "--field",
            "description=@/tmp/backplane-mr-body.md",
          ],
        }),
      );
    }),
