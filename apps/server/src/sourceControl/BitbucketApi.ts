...
import { sanitizeBranchFragment } from "@backplane/shared/git";
import {
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("BACKPLANE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("BACKPLANE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("BACKPLANE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("BACKPLANE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("@backplane/cli/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `backplane/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set BACKPLANE_BITBUCKET_EMAIL and BACKPLANE_BITBUCKET_API_TOKEN, or BACKPLANE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
