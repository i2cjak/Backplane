...
it.effect("launches an installed editor with platform-safe arguments", () =>
  Effect.gen(function* () {
    ...
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "backplane-editors-" });
...
// oxlint-disable-next-line backplane/no-global-process-runtime -- the skip decision needs the real host platform, outside any Effect runtime.
it.skipIf(process.platform !== "win32")(
  "delivers the raw /select switch for spaced paths through real PowerShell",
  { timeout: 60_000 },
  async () => {
    const tempDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "backplane-reveal-smoke-"));
    try {
  ...
  () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.gen(function* () {
        ...
        yield* launcher.launchEditor({
          editor: "file-manager",
          cwd: "/home/backplane/workspace/media/clip.mp4",
          reveal: true,
        });
        return { kind, editors };
      }).pipe(
      ...
      assert.equal(
        decodedCommand,
        "$ProgressPreference = 'SilentlyContinue'; Start-Process 'explorer.exe' -ArgumentList ('/select,\"' + '\\\\wsl.localhost\\Ubuntu-24.04\\home\\backplane\\workspace\\media\\clip.mp4' + '\"')",
      );
    }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
);
...
it.effect.skipIf(windowsHost)(
  ...
  () =>
    Effect.gen(function* () {
      ...
      assert.ok(launch);
      assert.deepEqual(launch.args, ["/home/backplane/workspace/media"]);
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* launcher.launchEditor({
          editor: "file-manager",
          cwd: '/home/backplane/work "quoted"/clip.mp4',
          reveal: true,
        });
      }).pipe(
        Effect.provide(
      ...
      assert.deepEqual(spawned.args, [
        '\\\\wsl.localhost\\Ubuntu-24.04\\home\\backplane\\work "quoted"',
      ]);
    }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
);
      ...
      Layer.mergeAll(
        ...
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: {
              PATH: "C:\\backplane-editor-discovery-cache-test",
              PATHEXT: ".COM;.EXE;.BAT;.CMD",
            },
          }),
        ),
    ...
    Effect.provide(
      Layer.mergeAll(
        ...
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: {
              PATH: "C:\\backplane-editor-discovery-interrupt-test",
              PATHEXT: ".COM;.EXE;.BAT;.CMD",
            },
          }),
        ),
