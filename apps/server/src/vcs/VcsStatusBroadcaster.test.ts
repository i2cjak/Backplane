...
import { GitManagerError } from "@backplane/contracts";
...
const remoteStatusWithPr: VcsStatusRemoteResult = {
  ...
  pr: {
    ...
    url: "https://github.com/i2cjak/backplane/pull/2978",
    baseRef: "main",
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const realDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "backplane-vcs-auto-pull-real-",
        });
        const linkParent = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "backplane-vcs-auto-pull-link-",
        });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const realDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "backplane-vcs-status-real-",
        });
        const linkParent = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "backplane-vcs-status-link-",
        });
