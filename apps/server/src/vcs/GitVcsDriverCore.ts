...
import { dedupeRemoteBranchesWithLocalMatches, normalizeGitRemoteUrl } from "@backplane/shared/git";
import { compactTraceAttributes } from "@backplane/shared/observability";
import { decodeJsonResult } from "@backplane/shared/schemaJson";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `backplane-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
    ...
    Effect.fn("refreshCheckedOutBranch")(function* (input) {
      ...
      if (!isAncestor) {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["update-ref", "refs/backplane/pre-refresh", headCommit],
