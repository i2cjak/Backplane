...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@backplane/contracts";
import { fromLenientJson } from "@backplane/shared/schemaJson";
...
>()("@backplane/cli/vcs/VcsProjectConfig") {}
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".backplane", "vcs.json");
