...
import { GitCommandError, type ReviewDiffFileContentsInput } from "@backplane/contracts";
import { ServerConfig } from "../config.ts";
import { makeGitVcsDriverCore, splitNullSeparatedGitStdoutPaths } from "./GitVcsDriverCore.ts";
import * as GitVcsDriver from "./GitVcsDriver.ts";

const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "backplane-git-vcs-driver-test-",
});
    ...
    it.effect("makes background upstream status fetches non-interactive", () =>
      Effect.gen(function* () {
        ...
        const envKeys = [
          ...
          "SSH_ASKPASS",
          "SSH_ASKPASS_REQUIRE",
          "BACKPLANE_TEST_SSH_ASKPASS_LOG",
        ] as const;
        ...
        yield* fileSystem.writeFileString(
          ...
          [
            "#!/bin/sh",
            'printf "GCM_INTERACTIVE=%s\\n" "${GCM_INTERACTIVE:-}" > "$BACKPLANE_TEST_SSH_ASKPASS_LOG"',
            'printf "GIT_ASKPASS=%s\\n" "${GIT_ASKPASS:-}" >> "$BACKPLANE_TEST_SSH_ASKPASS_LOG"',
            'printf "GIT_TERMINAL_PROMPT=%s\\n" "${GIT_TERMINAL_PROMPT:-}" >> "$BACKPLANE_TEST_SSH_ASKPASS_LOG"',
            'printf "SSH_ASKPASS=%s\\n" "${SSH_ASKPASS:-}" >> "$BACKPLANE_TEST_SSH_ASKPASS_LOG"',
            'printf "SSH_ASKPASS_REQUIRE=%s\\n" "${SSH_ASKPASS_REQUIRE:-}" >> "$BACKPLANE_TEST_SSH_ASKPASS_LOG"',
            "exit 1",
            "",
          ].join("\n"),
        );
        ...
        yield* Effect.gen(function* () {
          ...
          process.env.BACKPLANE_TEST_SSH_ASKPASS_LOG = sshLogPath;
  ...
  describe("remote operations", () => {
    it.effect("ensureRemote reuses an existing remote across ssh/https transport variants", () =>
      Effect.gen(function* () {
        ...
        yield* git(cwd, ["remote", "add", "origin", "https://github.com/i2cjak/backplane.git"]);

        const reusedForSsh = yield* driver.ensureRemote({
          cwd,
          preferredName: "i2cjak",
          url: "git@github.com:i2cjak/backplane.git",
        });
        ...
        const reusedForSshScheme = yield* driver.ensureRemote({
          cwd,
          preferredName: "i2cjak",
          url: "ssh://git@github.com/i2cjak/backplane",
        });
        ...
        const reusedForBareSshScheme = yield* driver.ensureRemote({
          cwd,
          preferredName: "i2cjak",
          url: "ssh://github.com/i2cjak/backplane",
        });
        ...
        const reusedForSshPort = yield* driver.ensureRemote({
          cwd,
          preferredName: "i2cjak",
          url: "ssh://git@github.com:22/i2cjak/backplane",
        });
        ...
        const reusedForSshWithPort = yield* driver.ensureRemote({
          cwd,
          preferredName: "i2cjak",
          url: "ssh://git@github.com:22/i2cjak/backplane.git",
        });
        ...
        const addedForFork = yield* driver.ensureRemote({
          ...
          url: "git@github.com:octocat/backplane.git",
        });
    ...
    it.effect("creates a worktree from the latest fetched remote commit", () =>
      Effect.gen(function* () {
        ...
        yield* driver.createWorktree({
          ...
          newRefName: "@backplane/cli/fetched-origin",
        ...
        assert.equal(
          yield* driver.readConfigValue(
            worktreePath,
            "branch.backplane/fetched-origin.gh-merge-base",
          ),
          initialBranch,
        );
        assert.equal(
          yield* driver.readConfigValue(worktreePath, "branch.backplane/fetched-origin.remote"),
          null,
        );
