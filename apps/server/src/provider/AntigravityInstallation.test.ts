...
const makeHarness = Effect.fn("test.makeAntigravityInstallation")(function* (
  ...
  const path = yield* Path.Path;
  const baseDir =
    options.baseDir ?? (yield* fs.makeTempDirectoryScoped({ prefix: "backplane-agy-test-" }));
  ...
  it.effect.skipIf(HostProcessPlatform.defaultValue() === "win32")(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "backplane-agy-path-test-" });
