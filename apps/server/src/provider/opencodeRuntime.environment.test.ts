  ...
  effectIt.live(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "backplane-opencode-output-" });
...
`,
        ...
        yield* fs.writeFileString(
          ...
          [
            ...
            isWindows
              ? '"%BACKPLANE_TEST_NODE_BINARY%" "%BACKPLANE_TEST_OPENCODE_SCRIPT%" %*'
              : 'exec "$BACKPLANE_TEST_NODE_BINARY" "$BACKPLANE_TEST_OPENCODE_SCRIPT" "$@"',
            "",
          ].join("\n"),
        );
        if (!isWindows) {
        ...
        const server = yield* runtime.startOpenCodeServerProcess({
          ...
          environment: {
            ...environment,
            BACKPLANE_TEST_NODE_BINARY: executablePath,
            BACKPLANE_TEST_OPENCODE_SCRIPT: scriptPath,
          },
        });
