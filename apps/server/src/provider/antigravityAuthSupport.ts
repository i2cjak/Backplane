...
import type { AntigravityAuthMethod, ProviderInstanceId } from "@backplane/contracts";
import { HostProcessExecutablePath, HostProcessPlatform } from "@backplane/shared/hostProcess";
...
export const ANTIGRAVITY_AUTH_BROWSER_MARKER = "__BACKPLANE_ANTIGRAVITY_AUTH_URL__";
...
const browserHelperSource =
  ...
  `()=>process.exit(0))`;
const browserPreflightUrl = "https://example.invalid/backplane-antigravity-browser-preflight";
...
/** Recognizes native auth failures and interactive login blocked by Backplane. */
...
/**
 * The agent reads its user-global skills under `GEMINI_HOME`, which Backplane points
...
export const prepareAntigravityProfile = Effect.fn("prepareAntigravityProfile")(function* (input: {
  ...
  if (
    ...
    return yield* authSupportError(
      "The Backplane runtime path cannot be used to suppress Antigravity browser launches.",
    );
  }
...
/** Receives native 1.1.1 sign-in URLs and Backplane browser-helper URLs without logging stderr. */
