...
const makeHarness = Effect.fn("makeAntigravityDriverHarness")(function* (
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "backplane-antigravity-driver-" });
  ...
  const instance = yield* AntigravityDriver.create({
    ...
    environment: [
      ...
      { name: "BACKPLANE_ACP_ANTIGRAVITY", value: "1" },
      { name: "BACKPLANE_ACP_REQUEST_LOG_PATH", value: requestLog },
...
const testLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "backplane-antigravity-driver-config-",
}).pipe(
