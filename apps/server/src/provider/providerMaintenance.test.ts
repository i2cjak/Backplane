...
import { ProviderDriverKind, ProviderInstanceId, type ServerProvider } from "@backplane/contracts";
...
it.layer(NodeServices.layer)("providerMaintenance", (it) => {
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("backplane-npm-capabilities");
  ...
  it.effect("proves Windows npm ownership from the package manifest beside the shim", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("backplane-npm-windows-capabilities");
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("backplane-pnpm-capabilities");
        const link = linkIntoPackage(tempDir, "package-tool", [
          ".local",
          "share",
          "pnpm",
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("backplane-bun-capabilities");
  ...
  it.effect.skipIf(windowsHost)("switches to native updates and runs the resolved executable", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("backplane-native-capabilities");
  ...
  it.effect.skipIf(windowsHost)("runs an explicit native updater outside PATH", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("backplane-native-update");
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const tempDir = yield* makeTempDir("backplane-homebrew-node-capabilities");
  ...
  it.effect.skipIf(windowsHost)("carries the native updater's environment into the action", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("backplane-native-env");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("backplane-missing-native-capabilities");
  ...
  it.effect.each([
    ...
    (fixture) =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("backplane-homebrew-capabilities");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("backplane-homebrew-foreign-prefix");
        const brewBinDir = NodePath.join(tempDir, "brew-bin");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("backplane-custom-capabilities");
