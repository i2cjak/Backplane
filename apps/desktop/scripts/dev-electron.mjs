...
const remoteDebuggingPort = process.env.BACKPLANE_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line backplane/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.BACKPLANE_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.BACKPLANE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--backplane-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--backplane-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
