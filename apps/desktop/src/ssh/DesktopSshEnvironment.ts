...
import * as SshAuth from "@backplane/ssh/auth";
import { resolveSshTarget } from "@backplane/ssh/command";
import { discoverSshHosts } from "@backplane/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@backplane/ssh/errors";
import * as SshTunnel from "@backplane/ssh/tunnel";
...
>()("@backplane/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliPackageSpec?: () => string;
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemoteBackplaneRunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Backplane window is not available for SSH authentication.";
      break;
