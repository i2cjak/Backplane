...
import type { DesktopPreviewRecordingFrame } from "@backplane/contracts";
...
const browserSessionLayer = Layer.succeed(
  ...
  BrowserSession.BrowserSession.of({
    getPartition: () => Effect.succeed("persist:backplane-preview-test"),
    isPartition: (partition) => partition.startsWith("persist:backplane-preview-"),
...
const environmentLayer = Layer.succeed(
  ...
  DesktopEnvironment.DesktopEnvironment.of({
    browserArtifactsDir: "/tmp/backplane/dev/browser-artifacts",
    dirname: "/tmp/backplane/desktop",
    path: {
...
describe("PreviewManager", () => {
  ...
  effectIt.effect("captures a PNG screenshot into browser artifacts", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        expect(capturePage).toHaveBeenCalledOnce();
        expect(mkdir).toHaveBeenCalledWith("/tmp/backplane/dev/browser-artifacts");
        ...
        expect(browserWindowConstructor).toHaveBeenCalledWith(
          expect.objectContaining({
            ...
            webPreferences: expect.objectContaining({
              preload: "/tmp/backplane/desktop/preview-pip-preload.cjs",
    ...
    withManager((manager) =>
      Effect.gen(function* () {
        yield* manager.revealArtifact(
          "/tmp/backplane/dev/browser-artifacts/browser-screenshot-test.png",
        );

        expect(showItemInFolder).toHaveBeenCalledWith(
          "/tmp/backplane/dev/browser-artifacts/browser-screenshot-test.png",
        );
        const exit = yield* Effect.exit(manager.revealArtifact("/tmp/backplane/dev/settings.json"));
        expect(Exit.isFailure(exit)).toBe(true);
        if (Exit.isSuccess(exit)) return;
        const error = Option.getOrThrow(Cause.findErrorOption(exit.cause));
        expect(error).toMatchObject({
          _tag: "PreviewArtifactPathOutsideDirectoryError",
          artifactPath: "/tmp/backplane/dev/settings.json",
          artifactDirectory: "/tmp/backplane/dev/browser-artifacts",
        });
  ...
  effectIt.effect("copies screenshot artifacts to the system clipboard", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        const artifactPath = "/tmp/backplane/dev/browser-artifacts/browser-screenshot-test.png";
        ...
        const exit = yield* Effect.exit(
          manager.copyArtifactToClipboard("/tmp/backplane/dev/settings.json"),
        );
        expect(Exit.isFailure(exit)).toBe(true);
        if (Exit.isSuccess(exit)) return;
        const error = Option.getOrThrow(Cause.findErrorOption(exit.cause));
