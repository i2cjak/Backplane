...
public final class BackplaneKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("BackplaneKeyboardCommands")

    View(BackplaneKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: BackplaneKeyboardCommandsView, commands: [String]) in
...
public final class BackplaneKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.backplaneFirstResponder == nil else { return }
...
private extension UIView {
  var backplaneFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.backplaneFirstResponder { return responder }
    }
    return nil
  }
}
