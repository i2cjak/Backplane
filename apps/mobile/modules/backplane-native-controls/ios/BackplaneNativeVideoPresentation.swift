...
final class BackplaneNativeVideoPresentation: NSObject, AVPlayerViewControllerDelegate,
  ...
  func present(from presenter: UIViewController, sources: BackplanePresentationSources, sourceIdentifier: String) {
    let audioSession = AVAudioSession.sharedInstance()
    previousAudioSession = AudioSessionConfiguration(audioSession)
    do {
      try audioSession.setCategory(.playback, mode: .moviePlayback)
    } catch {
      NSLog("Backplane video audio session: %@", error.localizedDescription)
    }
