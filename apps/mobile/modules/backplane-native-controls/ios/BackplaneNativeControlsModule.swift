...
public final class BackplaneNativeControlsModule: Module {
  private let presentationSources = BackplanePresentationSources()
  private var videoPresentation: BackplaneNativeVideoPresentation?
  private var filePresentation: BackplaneNativeFilePresentation?

  public func definition() -> ModuleDefinition {
    Name("BackplaneNativeControls")
    ...
    View(BackplanePresentationSourceView.self) {
      ViewName("PresentationSource")
      Prop("identifier") { (view: BackplanePresentationSourceView, identifier: String) in
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/BackplaneShowcaseScene"
    ...
    AsyncFunction("applyShowcaseOrientation") { (orientation: String) in
      ...
      for case let windowScene as UIWindowScene in UIApplication.shared.connectedScenes {
        windowScene.requestGeometryUpdate(.iOS(interfaceOrientations: mask)) { error in
          NSLog("BackplaneNativeControls applyShowcaseOrientation(\(orientation)) failed: \(error)")
        }
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/BackplaneShowcaseReadyScene"
  ...
  private func presentVideo(url: URL, title: String, sourceIdentifier: String, identifier: String, promise: Promise) throws {
    ...
    guard videoPresentation == nil, filePresentation == nil,
      ...
      throw NSError(
        domain: "BackplaneNativeVideo",
        code: 2,
        userInfo: [NSLocalizedDescriptionKey: "The video preview is no longer available."]
      )
    }
    let presentation = BackplaneNativeVideoPresentation(identifier: identifier, url: url, title: title) { [weak self] error in
  ...
  private func presentFile(url: URL, title: String, sourceIdentifier: String,
    ...
    let file = BackplaneNativeFilePresentation(identifier: identifier, sources: presentationSources,
  ...
  private func shareFile(url: URL, title: String, sourceIdentifier: String, promise: Promise) throws {
    guard let presenter = appContext?.utilities?.currentViewController() else {
      throw NSError(
        domain: "BackplaneNativePresentation",
        code: 2,
