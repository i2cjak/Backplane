# Backplane Mobile Terminal Native Module
...
custom-I/O and live-padding branch. `BackplaneTerminalView` owns a `libghostty` surface and uses that
callback I/O model:
...
`backplane/custom-io-ordered-feed` branch when vendored, based on VVTerm's
`vvterm/custom-io-padding` branch).

```bash
apps/mobile/modules/backplane-terminal/scripts/build-libghostty-ios16.sh
```
...
```bash
apps/mobile/modules/backplane-terminal/scripts/build-libghostty-android.sh
```
