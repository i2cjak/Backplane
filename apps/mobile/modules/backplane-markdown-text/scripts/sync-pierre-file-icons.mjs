...
const customSprite = webIconSource.match(/const BACKPLANE_FILE_ICON_SPRITE = `([\s\S]*?)`;/)?.[1];

if (!customSprite) {
  throw new Error(
    "Could not read the Backplane Pierre icon sprite from apps/web/src/pierre-icons.ts",
  );
}
...
const customIcons = {
  agents: "backplane-file-icon-agents",
  claude: "backplane-file-icon-claude",
  package: "backplane-file-icon-package-json",
  pnpm: "backplane-file-icon-pnpm",
  readme: "backplane-file-icon-readme",
  tsconfig: "backplane-file-icon-tsconfig",
  video: "backplane-file-icon-video",
};
