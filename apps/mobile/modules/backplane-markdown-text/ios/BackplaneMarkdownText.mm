#import "BackplaneMarkdownText.h"
#import "BackplaneMarkdownTextShadowNode.h"
#import "BackplaneMarkdownTextComponentDescriptor.h"
#import "BackplaneMarkdownTextRun.h"
#import <React/RCTConversions.h>
#import <objc/runtime.h>

#import <react/renderer/textlayoutmanager/RCTAttributedTextUtils.h>
#import <react/renderer/components/BackplaneMarkdownTextSpec/EventEmitters.h>
#import <react/renderer/components/BackplaneMarkdownTextSpec/Props.h>
#import <react/renderer/components/BackplaneMarkdownTextSpec/RCTComponentViewHelpers.h>
#import "RCTFabricComponentsPlugins.h"

using namespace facebook::react;

static void BackplaneMarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<BackplaneMarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void BackplaneMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<BackplaneMarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = BackplaneMarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        BackplaneMarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
...
@protocol BackplaneMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface BackplaneMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<BackplaneMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<BackplaneMarkdownOutsideTapTarget>)target;

@end

static const void *BackplaneMarkdownOutsideTapCoordinatorKey =
    &BackplaneMarkdownOutsideTapCoordinatorKey;

@implementation BackplaneMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<BackplaneMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<BackplaneMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, BackplaneMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<BackplaneMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static BackplaneMarkdownOutsideTapCoordinator *
BackplaneMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  BackplaneMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, BackplaneMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[BackplaneMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        BackplaneMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface BackplaneMarkdownText () <RCTBackplaneMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface BackplaneMarkdownText () <BackplaneMarkdownOutsideTapTarget>
@end

@implementation BackplaneMarkdownText {
  UIView * _view;
  UITextView * _textView;
  BackplaneMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<BackplaneMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const BackplaneMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    BackplaneMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, BackplaneMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [BackplaneMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<BackplaneMarkdownTextProps const>(_props);
  ...
  BackplaneMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  BackplaneMarkdownTextApplyAttachments(
      convertedAttrString,
      _state->getData().attachmentRanges,
      _attachmentImages);
  NSUInteger runLocation = 0;
  for (UIView *child in self.subviews) {
    if (![child isKindOfClass:[BackplaneMarkdownTextRun class]]) {
      continue;
    }

    BackplaneMarkdownTextRun *textChild = (BackplaneMarkdownTextRun *)child;
    ...
    NSURL *link = [NSURL URLWithString:
        [NSString stringWithFormat:@"backplane-markdown-run://%ld", (long)textChild.tag]];
    if (link != nil) {
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::BackplaneMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::BackplaneMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<BackplaneMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
...
{
  const auto &oldViewProps = *std::static_pointer_cast<BackplaneMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<BackplaneMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == BackplaneMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == BackplaneMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == BackplaneMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == BackplaneMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const BackplaneMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (nullable BackplaneMarkdownTextRun *)childForCharacterRange:(NSRange)characterRange
{
...
- (BackplaneMarkdownTextRun*)getTouchChild:(CGPoint)location
{
...
{
  BackplaneMarkdownTextRun *child = [self childForCharacterRange:textItem.range];
  if (![child hasContextMenu]) {
    return defaultAction;
  }

  __weak BackplaneMarkdownTextRun *weakChild = child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::BackplaneMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::BackplaneMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> BackplaneMarkdownTextCls(void)
{
  return BackplaneMarkdownText.class;
}
