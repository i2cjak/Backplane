...
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@backplane/contracts";
...
describe("buildHomeThreadGroups", () => {
  it("builds one v2 scope for the same repository across environments", () => {
    ...
    const repositoryIdentity = {
      ...
      locator: {
        ...
        remoteName: "origin",
        remoteUrl: "git@github.com:i2cjak/backplane.git",
      },
    };
    const projects = [
      makeProject({
    ...
    expect(scopes[0]?.title).toBe("backplane");
  ...
  it("routes stale duplicate project refs through the canonical repository group", () => {
    ...
    const local = makeProject({
      ...
      environmentId: localEnvironmentId,
      title: "backplane",
      workspaceRoot: "/workspaces/backplane",
      repositoryIdentity,
    });
    const stale = makeProject({
      ...
      title: "backplane",
      workspaceRoot: "/remote/backplane",
      updatedAt: "2026-06-01T00:00:00.000Z",
    });
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    ...
    const repositoryIdentity = {
      ...
      locator: {
        source: "git-remote" as const,
        remoteName: "origin",
        remoteUrl: "git@github.com:backplane/backplane.git",
      },
      provider: "github",
      owner: "backplane",
      name: "backplane",
      displayName: "Backplane",
      rootPath: "/workspaces/backplane",
    };
    const projects = [
      makeProject({
        environmentId,
        ...
        title: "Web",
        workspaceRoot: "/workspaces/backplane/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/backplane/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
  ...
  it("does not apply the recency window while searching", () => {
    ...
    const group = buildGroups([project], threads, { searchQuery: "Backplane" })[0];
