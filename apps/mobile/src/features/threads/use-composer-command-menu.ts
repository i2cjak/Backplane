import type { EnvironmentId, ProviderInteractionMode, ServerProvider } from "@backplane/contracts";
import { USAGE_LIMITS_COMMAND } from "@backplane/shared/usageLimits";
import {
  detectComposerTrigger,
  replaceTextRange,
  serializeComposerFileLink,
  type ComposerTrigger,
} from "@backplane/shared/composerTrigger";
import {
  insertRankedSearchResult,
  normalizeSearchQuery,
  scoreQueryMatch,
} from "@backplane/shared/searchRanking";
import {
  dedupeProviderSkillsByName,
  getProviderSkillsForSlashMenu,
  isProviderSkillUserInvocable,
  resolveProviderSkillsForCwd,
} from "@backplane/client-runtime/providerSkills";
...
export function buildComposerSlashCommandItems(input: {
  ...
  /** Whether Backplane itself offers /usage-limits for the selected provider. */
  ...
  // Providers expand commands only at the start of a message. Backplane commands
  ...
  for (const command of input.selectedProviderStatus?.slashCommands ?? []) {
    ...
    // Backplane's own limits command is answered by the thread composer; New Task has
