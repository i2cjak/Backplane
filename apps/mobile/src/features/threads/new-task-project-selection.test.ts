...
function makeScope(projects: ReadonlyArray<EnvironmentProject>): HomeProjectScope {
  return {
    key: "github.com/backplane/backplane",
    title: "Backplane",
...
describe("getProjectScopeSelectionTarget", () => {
  it("keeps the current environment when it hosts the selected logical project", () => {
    const projects = [
      makeProject("backplane-mac", "mac"),
      makeProject("backplane-server", "server"),
    ];
...
describe("resolveEnvironmentProjectMatch", () => {
  it("follows the same repository onto the target machine", () => {
    const selected = makeProject("backplane", "mac", {
      repositoryKey: "github.com/backplane/backplane",
    });
    const target = [
      makeProject("other", "server", { repositoryKey: "github.com/backplane/other" }),
      makeProject("backplane-clone", "server", { repositoryKey: "github.com/backplane/backplane" }),
    ];
  ...
  it("falls back to workspace basename, then title, for unindexed projects", () => {
    const selected = makeProject("backplane", "mac", { workspaceRoot: "/Users/me/backplane" });
    const byBasename = [
      makeProject("other", "server"),
      makeProject("srv", "server", { workspaceRoot: "/home/me/backplane" }),
    ];
    expect(resolveEnvironmentProjectMatch(byBasename, selected)).toBe(byBasename[1]);

    const byTitle = [
      makeProject("other", "server"),
      makeProject("srv", "server", { title: "backplane" }),
    ];
  ...
  it("does not treat a known different repository as a basename or title match", () => {
    const selected = makeProject("backplane", "mac", {
      repositoryKey: "github.com/backplane/backplane",
      workspaceRoot: "/Users/me/backplane",
    });
    const fork = makeProject("fork", "server", {
      repositoryKey: "github.com/someone/backplane",
      title: "backplane",
      workspaceRoot: "/home/me/backplane",
    });
    const unindexed = makeProject("unindexed", "server", { workspaceRoot: "/srv/backplane" });
...
describe("resolveDraftProjectSelection", () => {
  it("preserves an explicit project selection", () => {
    const project = makeProject("backplane");
    expect(
      resolveDraftProjectSelection("environment:backplane", [project], [makeScope([project])]),
  ...
  it("selects one logical project even when it has multiple physical workspaces", () => {
    const projects = [
      makeProject("backplane"),
      makeProject("backplane-2"),
      makeProject("backplane-3"),
    ];
