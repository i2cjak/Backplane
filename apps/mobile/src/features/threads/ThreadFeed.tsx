...
import { renderAssistantCitationsAsText } from "@backplane/shared/assistantCitations";
import {
...
import { formatAttachmentSize } from "@backplane/client-runtime/state/attachments";
...
import { resolveViewedImageAsset } from "@backplane/client-runtime/work-log/presentation";
import {
...
} from "@backplane/shared/chatList";
...
import { markdownFileIconSource } from "@backplane/mobile-markdown-text/file-icons";
import { markdownLinkIconSource } from "@backplane/mobile-markdown-text/link-icons";
import {
  normalizeNativeMarkdownUrl,
  resolveMarkdownInlineCodePresentation,
  resolveMarkdownLinkIcon,
  resolveMarkdownLinkPresentation,
} from "@backplane/mobile-markdown-text/links";
import {
    ...
    >
      ...
      >
        ...
        <View className="min-w-0 flex-1 gap-1">
          <Text className="font-backplane-medium text-sm text-foreground" numberOfLines={2}>
            {attachment.name}
          </Text>
...
function MarkdownInlineCode(props: {
  ...
  return (
    <NativeText
      className={presentation ? "font-backplane-bold" : "font-mono"}
  ...
  return (
    <View className="my-2 min-w-0 flex-row items-center gap-3 rounded-2xl border border-border bg-card px-3 py-3">
      ...
      <View className="min-w-0 flex-1">
        <Text className="font-backplane-bold text-sm text-foreground" numberOfLines={1}>
      ...
      {props.onUse ? (
        ...
        >
          <Text className="font-backplane-bold text-xs text-foreground">Use template</Text>
        </Pressable>
      ) : null}
    </View>
  );
    ...
    return (
      ...
      >
        <Text
          ...
          className="font-backplane-medium text-sm tabular-nums text-foreground-muted"
        >
          {entry.label}
        </Text>
        <ThreadDisclosureChevron
      ...
      >
        ...
        <View className="shrink-0 flex-row items-center gap-1.5">
          <SymbolView
            ...
            type="monochrome"
          />
          <Text className="font-backplane-medium text-xs text-foreground-muted">{label}</Text>
        </View>
        <View className="h-px flex-1 bg-adaptive-neutral-200-a80-white-a8" />
      </View>
    );
      ...
      return (
        ...
        >
          ...
          <View className="mt-1 flex-row items-center justify-end gap-1 pr-0.5">
            <Text className="font-backplane-medium text-xs tabular-nums text-adaptive-neutral-600-400">
              {timestampLabel}
            </Text>
            {message.text.trim().length > 0 ? (
              <CopyTextButton
  ...
  return (
    ...
    >
      <View className="max-w-[320px] items-center gap-2">
        <Text className="text-center font-backplane-bold text-lg text-foreground">
          {props.title}
        </Text>
