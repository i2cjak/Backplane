    ...
    <View className="gap-3 rounded-[24px] border-continuous bg-card p-4">
      <View className="flex-row items-start justify-between gap-3">
        <View className="gap-1">
          <Text className="text-sm font-backplane-medium text-foreground">{pool.label}</Text>
          <View className="flex-row items-baseline gap-1.5">
            <Text className="text-3xl font-backplane-bold tabular-nums text-foreground">
        ...
        {pool.members.map(({ account, window }, index) => (
          ...
          >
            ...
            <View pointerEvents="none" className="absolute inset-0 items-center justify-center">
              <Text className="text-xs font-backplane-medium tabular-nums text-foreground">
                {index + 1}
              </Text>
            </View>
          </Pressable>
        ...
        {pool.members.map(({ account, window }, index) => {
          ...
          return (
            ...
            >
              ...
              <Text
                numberOfLines={1}
                className="min-w-0 flex-1 text-sm font-backplane-medium text-foreground"
              >
                {accountName(account)}
              </Text>
              <Text className="text-sm font-backplane-medium tabular-nums text-foreground">
      ...
      {pools.map((pool) => (
        <View key={pool.driver} className="gap-3">
          <View className="flex-row items-center gap-2 px-1">
            ...
            <Text className="text-base font-backplane-medium text-foreground">
          ...
          <>
            <View className="gap-2">
              <View className="flex-row items-center gap-2">
                ...
                <Text className="flex-1 text-xl font-backplane-bold text-foreground">
            ...
            <View className="gap-3 rounded-[24px] border-continuous bg-card p-4">
              <Text className="text-sm font-backplane-medium text-foreground">{window.label}</Text>
              <Text className="text-3xl font-backplane-bold tabular-nums text-foreground">
            ...
            <View className="gap-2 rounded-[24px] border-continuous bg-card p-4">
              <Text className="text-sm font-backplane-medium text-foreground">
            ...
            {account.redeem && account.limits.resetCredits ? (
              <View className="gap-3 rounded-[24px] border-continuous bg-card p-4">
                <Text className="text-sm font-backplane-medium text-foreground">Reset credits</Text>
                <ResetCredits
