...
- `backplane-relay-traces-prod`, the OpenTelemetry trace dataset shared by the Worker, mobile app, and
  first-party relay clients
- `backplane-relay-otel-ingest-prod`, the dataset-scoped Worker ingest token
- `backplane-mobile-otel-ingest-prod`, the dataset-scoped mobile ingest token
- `backplane-relay-client-otel-ingest-prod`, the dataset-scoped first-party relay-client ingest token
- `backplane-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`backplane-relay-traces-dev-julius` for a personal stage.
