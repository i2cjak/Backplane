...
Connections**, sign in, and enable **Backplane Connect** for that environment.
...
Backplane Connect renews access credentials when needed without disconnecting a healthy
...
[app.backplane.works](https://app.backplane.works) needs an HTTPS endpoint. It connects directly
...
**SSH**, and enter a host or SSH alias such as `user@example.com`. Backplane starts
...
the connection stops a server that Backplane launched; a server that was already
...
To remove an environment from Backplane Connect, open your account menu's **Backplane Connect**
page, or **Settings → Backplane Connect** on mobile, and choose **Deregister**. This
revokes its cloud access and frees its host space even when the environment is
offline or has been wiped.

On a command-line host, `backplane connect unlink` disables exposure while retaining
your login; `backplane connect logout` also clears that login. Background-service
...
## Backplane Connect troubleshooting

Run `backplane connect status` on the host to inspect saved authorization and link
configuration. It is not a live reachability check. If the environment appears
offline, run `backplane service status` and read the displayed log. If it disappears
when SSH closes, see [background-service troubleshooting](./background-service.md#troubleshooting).

| Error                                                     | Recovery                                                                                                                                                         |
| --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment_link_limit_exceeded` or managed tunnel limit | Deregister an unused environment, then restart Backplane on the host.                                                                                            |
| `auth_invalid` or `invalid_bearer`                        | Run `backplane connect login`. If credentials were revoked, run `backplane connect logout`, then `backplane connect` again. Restart the server after signing in. |
| Expired or invalid link proof                             | Check the host's date and time, update Backplane, then restart it.                                                                                               |
...
`systemctl --user restart backplane.service` for the background service. For a
foreground server, stop it and run `backplane serve` again with your usual options.
...
devices. For server version warnings, follow [Updating Backplane](./updating.md).
