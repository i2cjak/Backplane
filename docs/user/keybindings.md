...
`~/.backplane/userdata/keybindings.json` by default. You can edit this file directly.
...
Backplane creates the file with its defaults and adds new defaults on later startups.
...
Invalid rules are ignored; if the file cannot be parsed, Backplane uses defaults.
