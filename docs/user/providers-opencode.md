...
Backplane requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let Backplane start OpenCode locally. A password in
provider settings applies to both that server and Backplane's connection. With no
...
settings. Backplane uses only that configured password for an external server; it
...
restart before Backplane can see configuration changes.
