...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="Backplane-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit backplane-icon.png
git config user.name 'backplane-ci'
git config user.email 'backplane-ci@users.noreply.github.com'
git add -A
