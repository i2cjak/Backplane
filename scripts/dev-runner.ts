...
import { resolveGitWorktreePath, resolveWorktreeBackplaneHome } from "@backplane/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@backplane/shared/hostProcess";
...
export const DEFAULT_BACKPLANE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
...
const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@backplane/contracts",
    "--filter=@backplane/web",
    "--filter=@backplane/cli",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=@backplane/cli", "dev"],
  "dev:web": ["run", "--filter=@backplane/web", "dev"],
  "dev:desktop": ["run", "--filter=@backplane/desktop", "--filter=@backplane/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`BACKPLANE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("BACKPLANE_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("BACKPLANE_PORT_OFFSET"),
  devInstance: optionalStringConfig("BACKPLANE_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "BACKPLANE_PORT_OFFSET",
    ...
    return Effect.succeed({
      ...
      source: `BACKPLANE_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric BACKPLANE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed BACKPLANE_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_BACKPLANE_HOME;
  });
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .backplane > ambient BACKPLANE_HOME) is resolved
    // by the caller; an unset backplaneHome here genuinely means "use the default".
    const configuredBaseDir = backplaneHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.BACKPLANE_HOME = resolvedBaseDir;
    } else {
      delete output.BACKPLANE_HOME;
    }

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed backplane service (an
    // agent working inside Backplane), these leak through and the child server
    // fails startup with "The service launcher started a different backplane version"
    // (serviceLauncherClient.ts resolveStartup).
    delete output.BACKPLANE_SERVICE_LAUNCHER_CONTEXT;
    delete output.BACKPLANE_BOOT_SERVICE_UNIT;

    if (!isDesktopMode) {
      output.BACKPLANE_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.BACKPLANE_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.BACKPLANE_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.BACKPLANE_PORT = String(serverPort);
      ...
      delete output.BACKPLANE_SINGLE_ORIGIN_DEV;
      delete output.BACKPLANE_MODE;
      delete output.BACKPLANE_NO_BROWSER;
      delete output.BACKPLANE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.BACKPLANE_HOST = host;
    }

    if (!isDesktopMode) {
      output.BACKPLANE_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.BACKPLANE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.BACKPLANE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.BACKPLANE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.BACKPLANE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.BACKPLANE_MODE = "web";
      delete output.BACKPLANE_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`BACKPLANE_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["BACKPLANE_PORT_OFFSET", "BACKPLANE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.backplane` — see @backplane/shared/devHome for why this must
    // outrank an ambient BACKPLANE_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreeBackplaneHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedBackplaneHome =
      (input.backplaneHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.BACKPLANE_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      backplaneHome: resolvedBackplaneHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.BACKPLANE_HOME ?? (yield* DEFAULT_BACKPLANE_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.BACKPLANE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.BACKPLANE_DEV_ALLOWED_ORIGINS = [
            env.BACKPLANE_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so BACKPLANE_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.BACKPLANE_BUNDLED_DEV === undefined) {
            env.BACKPLANE_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  backplaneHome: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Backplane data directory; runtime state is stored under userdata (equivalent to BACKPLANE_HOME). Inside a git worktree this defaults to that worktree's own .backplane so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to BACKPLANE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("BACKPLANE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to BACKPLANE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("BACKPLANE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to BACKPLANE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("BACKPLANE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to BACKPLANE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("BACKPLANE_PORT")),
  ),
