// @effect-diagnostics nodeBuiltinImport:off globalTimers:off globalDate:off - This host-side fixture creates an isolated local Backplane environment.
...
export const SHOWCASE_PROJECT_ID = "backplane";
...
const SHOWCASE_TERMINAL_PROMPT =
  "\u001b[1;32m→\u001b[0m \u001b[1;36mbackplane\u001b[0m \u001b[1;34mgit:(\u001b[1;31mfeat/remote-command-center\u001b[1;34m)\u001b[0m \u001b[1;33m✗\u001b[0m ";
...
export const SHOWCASE_TERMINAL_BUFFER = [
  ...
  "  \u001b[32m→\u001b[0m  \u001b[2mProject:\u001b[0m  \u001b[1mbackplane\u001b[0m \u001b[2m— ~/Code/backplane\u001b[0m",
  "",
...
const PROJECT_FAVICONS = {
  backplane: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
...
export const SHOWCASE_PROJECTS = [
  {
    ...
    title: "Backplane",
    directory: "backplane",
    repositoryUrl: "https://github.com/i2cjak/backplane.git",
    favicon: PROJECT_FAVICONS.backplane,
  },
  {
    id: "react",
    title: "React",
...
async function seedBackplaneWorkspace(workspaceRoot: string): Promise<void> {
  ...
  await NodeFSP.writeFile(
    ...
    `${JSON.stringify({ name: "backplane", private: true, scripts: { test: "vp test" } }, null, 2)}\n`,
  );
  await NodeFSP.writeFile(NodePath.join(workspaceRoot, "favicon.svg"), PROJECT_FAVICONS.backplane);
  await NodeFSP.writeFile(
...
async function seedCompanionWorkspace(input: {
  ...
  await NodeFSP.writeFile(
    ...
    `# ${input.title}\n\nSeeded by the Backplane mobile screenshot harness.\n`,
  );
  await initializeRepository({
...
export async function seedShowcaseEnvironment(input: {
  ...
  if (primaryProject.id === SHOWCASE_PROJECT_ID) {
    await seedBackplaneWorkspace(workspaceRoot);
  }
  await Promise.all(
    projects
