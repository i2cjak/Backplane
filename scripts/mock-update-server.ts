...
const resolveMockUpdateServerConfig = Effect.gen(function* () {
  ...
  const config = yield* Config.all({
    port: Config.port("BACKPLANE_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.withDefault(3000)),
    root: Config.string("BACKPLANE_DESKTOP_MOCK_UPDATE_SERVER_ROOT").pipe(
