...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("backplane-relay-traces", "prod")).toBe(
      "backplane-relay-traces-prod",
    );
    expect(relayResourceNameForStage("backplane-relay-traces", "dev_julius")).toBe(
      "backplane-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "backplanerelay.com", hash)).toBe(
      "prod-abcdef0123456789.backplanerelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "backplanerelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
