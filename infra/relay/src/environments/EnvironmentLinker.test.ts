...
import { RELAY_LINK_PROOF_TYP } from "@backplane/shared/relayJwt";
...
const makeRequest = Effect.gen(function* () {
  ...
  const payload = {
    iss: "backplane-env:env-link-test",
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.succeed(EnvironmentCredentials.EnvironmentCredentials, {
          create: () => Effect.succeed("backplaneenv_credential_secret"),
...
describe("EnvironmentLinker", () => {
  it.effect("uses verified JWT claims when linking an environment", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(result.environmentCredential).toBe("backplaneenv_credential_secret");
