...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    BACKPLANE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    BACKPLANE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    BACKPLANE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    BACKPLANE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    BACKPLANE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    BACKPLANE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("BACKPLANE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("BACKPLANE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "BackplaneRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Backplane relay through Alchemy."));
