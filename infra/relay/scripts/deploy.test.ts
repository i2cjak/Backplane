...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "backplane-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "backplane-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "BACKPLANE_MOBILE_OTLP_TRACES_DATASET=backplane-mobile-traces-dev",
        "BACKPLANE_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "BACKPLANE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "BACKPLANE_RELAY_CLIENT_OTLP_TRACES_DATASET=backplane-relay-client-traces-dev",
        "BACKPLANE_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
  ...
  it("replaces stale values while preserving unrelated entries", () => {
    expect(
      reconcileRootEnvPublicConfig(
        [
          "BACKPLANE_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "BACKPLANE_RELAY_URL=https://old.example.test",
          "BACKPLANE_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "BACKPLANE_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "BACKPLANE_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "BACKPLANE_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "BACKPLANE_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "BACKPLANE_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "BACKPLANE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "BACKPLANE_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "BACKPLANE_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
